...
refresh scriptはWSL/Docker起動後の現在IPへportproxyを張り直す。Windows側のlisten addressは `192.168.0.99`、Firewallの送信元は `192.168.0.0/24` に限定し、旧 `0.0.0.0` mappingを残さない。keepaliveは60秒間隔で、3220/3221と設定済み3222について「mappingあり・LAN listenerなし」の時だけIP HelperをStart/Restartする。さらに通常WSL shellとsystemd PID 1の両方から `/mnt/e/GitHub/RukalunPage` を確認し、通常shellは成功してPID 1だけ厳密に `Invalid argument` となるmount namespace不整合が2回連続した場合だけ、試行時刻を `E:\GitHub\BotManager\logs\dokploy-wsl-mount-recovery-state.json` へ先に保存して `Ubuntu-Backup` をterminateする。入れ子quoteをPowerShell 5.1のnative引数処理で壊さないよう、probe本体はUTF-8/base64 wrapper経由で `bash -lc` へ渡す。state保存失敗や別のprobe errorではterminateしない。terminateにより同distribution上の全Docker/Swarm/SUB AI serviceが一時停止する。terminate後10秒で既存loopへ戻し、正常probe後だけ復旧成功とする。試行後15分はTask再起動後も再terminateしない。mount元自体が読めない場合、mapping欠落、HTTP/app障害は別障害として扱い、WSLやIP Helperを再起動しない。

反映時は両ファイルのSHA-256が追跡元と一致すること、Taskが `Running` であること、必要な場合はkeepaliveログが `pending` → `stale; terminating` → `terminate result=success` → 正常probe後の `recovery result=success` となること、Bot serviceが `1/1` へ復帰したこと、listenerとLAN APIを確認する。

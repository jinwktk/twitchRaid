...
| Vercel | `../RukalunPage/index.html` / `scripts/export-clip-search-data.mjs` | SQLite Clipキャッシュから公開用JSONを生成し、別リポジトリの静的ページでClip検索を提供 |
...
- VercelのClip検索画面は静的配信のため、`data/clips.sqlite` へ直接アクセスしない。`scripts/export-clip-search-data.mjs` で `RukalunPage/clip-search-data.json` を生成し、公開項目はClip ID、URL、タイトル、作成者表示名、ゲーム名、サムネイルURL、作成日、再生数、`clipSync.recentSyncedAt` のみに限定する。必要時は `--enrich-from-twitch` でTwitch APIからサムネイルURLとゲーム名を補完する。Botの自動公開は `CLIP_SEARCH_PUBLISH_REPO_DIR` でRukalunPage repoをGit操作先にできる。画面上のClip最終同期時刻はJSTの秒単位で表示し、各カードはサムネイル/ゲーム名と `Twitchで見る` 外部リンクだけを表示する。

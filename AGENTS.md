...
- `src/docs/clip-search-data-publisher.ts`: Botの直近Clip同期完了後に、サブPCのSQLite DBから `docs/clip-search-data.json` を再生成し、差分があれば `main` へcommit/pushする任意機能。保存0件の同期も既定5分ごとに公開し、Clip保存があった場合は間隔内でも公開する。Bot再起動時は既存JSONの `generatedAt` を読んで公開間隔を引き継ぐ。
...
- TDD: `tests/docs-clip-search-page.test.ts` に `NEW_CLIP_WINDOW_MS`、3日ミリ秒定数、`isNewClip`、`new-badge`、`aria-label="新着Clip"`、`NEW` 表示の期待値を追加し、未実装失敗を確認
- 実装: `docs/clip-search.html` に `createdAt` と現在時刻の差が72時間未満かつ未来日時ではない場合だけtrueになる `isNewClip` を追加。各カードのリボン左側を日付と `NEW` バッジの横並びにし、CSSで小さな丸形バッジとして表示する。公開JSONの項目追加はなし
- ドキュメント: READMEとAGENTSに、Clip検索カードは作成日時が3日以内のClipへ `NEW` マークを付ける仕様を追記
- 検証: `npm test -- --run tests/docs-clip-search-page.test.ts` 9件、`npm test` 184件、`npm run build`、`npm run lint`、`python -m pytest -q` 107件、HTMLParserによる `docs/clip-search.html` / `docs/index.html` / `docs/typescript-bot-spec.html` / `internal-docs/twitchraid-bot-zukan.html` 構文確認、`git diff --check` が通過。確認用の一時公開JSONでPlaywright表示確認を行い、1280px幅/390px幅とも新着カードだけ日付横に `NEW` が出て、古いカードには出ず、SP検索トグルの右端 `▽` と横幅が崩れないことを確認
...
- 検証: `npm test` 179件、`npm run build`、`npm run lint`、`python -m pytest -q` 107件、HTMLParserによる `docs/clip-search.html` / `docs/index.html` / `docs/typescript-bot-spec.html` / `internal-docs/twitchraid-bot-zukan.html` 構文確認、公開JSONキー/件数/補完件数確認、`git diff --check` が通過。追加で `npm test -- --run tests/docs/clip-search-data-publisher.test.ts` 5件、`npm run build`、`npm run lint` が通過し、再起動後の間隔引き継ぎを確認
...
- 検証: `npm test -- --run tests/docs-clip-search-page.test.ts` 8件、`npm test` 174件、`npm run build`、`npm run lint`、`python -m pytest -q` 107件、HTMLParserによる `docs/clip-search.html` / `docs/index.html` / `docs/typescript-bot-spec.html` / `internal-docs/twitchraid-bot-zukan.html` 構文確認、`git diff --check` が通過。ローカルHTTPサーバーとPlaywrightで390px幅の初期折りたたみ、開閉後の `aria-expanded` / 検索グリッド表示切替、`68 / 2,750 clips` 表示、横はみ出しなし、1024px幅ではトグル非表示かつ検索条件常時表示を確認

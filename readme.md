...
## 更新履歴
- **2026-07-20**: `!chat タン塩？塩タン？` の直後に `!chat 調べて？` と送っても、直前の話題を検索せず「何について？」と聞き返す問題を修正。対象を省略した短い検索依頼だけ、同じ依頼者による安全な直前AI会話発言を検索語へ使い、会話履歴と検索結果を同じ回答promptへ渡す。別ユーザーの割込み、unsafeな直前発言、複数の独立質問を検索へ誤転用しない回帰テストも追加した。

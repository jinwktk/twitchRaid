...
## 更新履歴
- **2026-06-21**: 既存 `twitchRaid Memory` WebUIの内部データソースをSQLite正本からself-host Mem0 OSSへ切り替えた。`scripts/memory-web.mjs` は従来通り `127.0.0.1:3220` / `/api/memory` で動き、既定では `ssh sub` 経由でSUB AI Services内の `sub-ai_mem0` コンテナへ入り、APIキーを外へ出さずに一覧、semantic search、新規追加、本文更新、削除を行う。検索欄に文字が残っていても `List` は通常一覧、`Semantic Search` だけsemantic searchとして扱う。`ops/mem0-oss-server/app.py` は管理用に `PATCH /memories/{id}` と `DELETE /memories/{id}` を追加した。mem0にはinactive状態がないため、旧SQLite画面のactive/inactive切替は廃止した

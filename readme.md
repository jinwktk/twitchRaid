...
- `CHAT_AI_IMPLICIT_MEMORY_ENABLED=true` を追加すると、明示的に「覚えて」と言わない通常AIメンションでも、返信送信に成功した後で安全な短文だけをルールベース抽出して記憶ストアへ保存します。対象は `私はカレーが好き`、`私は社会人だよ`、`るっかは43歳`、`るっかの好きなゲームはVALORANT` のような安定した事実・嗜好に限定し、疑問文、依頼文、今日/昨日などの一時情報、会話の打ち切りや勝ち負けなどの一時反応、なぞなぞ、Twitchエモート混じりのBot返答、秘密情報、個人情報キー、プロンプト注入文は保存しません。一人称の一般属性は発言者名へ正規化しますが、`私は43歳だよ` のような年齢・生年月日・居住地系は保存しません。暗黙保存は追加のOllama呼び出しや追加prompt生成を行わず、チャット返信後に非同期で実行します。保存権限と上限は `CHAT_AI_AUTO_LEARN_ENABLED` / `CHAT_AI_MEMORY_WRITER_USERS` / `CHAT_AI_AUTO_LEARN_MAX_*` と同じです。`CHAT_AI_MEM0_ENABLED=true` の場合は、ローカルJSON/SQLite保存が失敗しても安全抽出済みの暗黙メモをself-host mem0へ `infer:false` で保存します。保存されたメモは `kind=implicit` になり、次回以降 `CHAT_AI_MEMORY_ENABLED=true` の関連取得でだけプロンプトへ入ります
- `CHAT_AI_COMMENT_MEMORY_ENABLED=true` を追加すると、`!chat` やBot宛てメンションではない通常の配信コメントも、チャット返信やOllama呼び出しを行わず非同期で記憶候補として読みます。既定は無効です。保存するのは通常コメント本文そのものではなく、抽出済みの `key: value` だけです。対象は `私はカレーが好き`、`私は社会人だよ`、`カレーが好き` のような発言者本人の安定した自己紹介・嗜好と、`るっか` / `るっかるん` / `rukalun` / `にめいや` / `にめいやボットくん` / `nyme_ia2` など既知対象の安定事実に限定します。質問、依頼、一時的な近況、なぞなぞ、会話打ち切り、悪口、Twitchエモート混じり、URL、メール、電話番号、token/API key/password、プロンプト注入、一人称の年齢・生年月日・居住地は保存しません。保存権限とストアは `CHAT_AI_AUTO_LEARN_ENABLED` / `CHAT_AI_IMPLICIT_MEMORY_ENABLED` / `CHAT_AI_MEMORY_WRITER_USERS` / `CHAT_AI_MEMORY_STORE` に従い、`CHAT_AI_MEM0_ENABLED=true` の場合はself-host mem0へも `infer:false` でミラーします。`CHAT_AI_COMMENT_MEMORY_MAX_ENTRIES_PER_MESSAGE` は既定 `2`、`CHAT_AI_COMMENT_MEMORY_DEDUP_TTL_SECONDS` は既定 `21600` で、同一 `sourceUser/key/value` のmem0重複保存を6時間抑制します

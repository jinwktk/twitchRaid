...
## 更新履歴
- **2026-06-21**: `CHAT_AI_MEMORY_WRITER_USERS=all` を全ユーザー許可として扱うようにした。`tests/bot-mention-chat.test.ts` に任意ユーザーの記憶保存が固定返信 `覚えたD！` で成功し、Ollamaを呼ばない回帰テストを追加した。サブPCDokploy本番の `application.env` も `CHAT_AI_MEMORY_WRITER_USERS=all` へ更新する

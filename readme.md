...
- **2026-07-27**: `!chat 今日の天気を教えて` が検索0件、または検索後も「天気は知らない」と返る問題を修正した。自然な情報依頼の末尾を検索語から除く際に `教えて` だけでなく直前の助詞 `を` も同時に除き、`今日の天気` としてSearXNGへ送る。さらに日本語の `今日の天気` を一語の完全一致として要求していた関連度判定を天気検索だけ補正し、相対日は無視しつつ明示地域と `天気` は必須にした。検索スニペットが予報サイトの案内だけで具体値を持たない場合に備え、許可したHTTPSのtenki.jp日別ページからJSON-LDの地域、今日の予報、最高/最低気温、予報日だけを既存上限内で取得し、channel workspaceのRAGを維持したままAnythingLLMへ渡す。検索0件固定返信で終了しないこと、別地域の結果を採用しないこと、具体的な予報値をBot経由で渡すことを単体・統合テストで固定した。

...
## 更新履歴
- **2026-07-05**: `CHAT_AI_PROMPT_REPLY_LOG_ENABLED=true` の成功時全文診断ログをINFOではなくSUCCESSログへ変更し、ヘッダも `AIメンション会話プロンプト/返信:` から `AIメンション会話プロンプト/Success:` へ変更した。さらにログビューアで先頭行だけSUCCESS、後続prompt行がINFOへ分裂しないよう、成功/失敗の全文診断ログは改行を `\n` にエスケープした1行形式へ変更した。
...
- **2026-07-05**: `CHAT_AI_PROMPT_REPLY_LOG_ENABLED=true` の全文診断ログを、成功時だけでなくtimeout、HTTPエラー、無効応答、policy rejected、例外などの失敗時にも出すようにした。失敗時は `AIメンション会話プロンプト/失敗:` に `理由="..."` と実際にOllamaへ送った構築済み `プロンプト="..."` を出し、timeout fallbackを返す場合は `フォールバック返信="..."` も併記する。対象テストは `tests/commands/mention-chat.test.ts` に追加した。
...
- **2026-06-20**: `CHAT_AI_PROMPT_REPLY_LOG_ENABLED=true` を追加。AIメンション会話の成功時に、Ollamaへ送った構築済みprompt全文と生成返信をログへ出せるようにした。現在は `プロンプト="..."` / `返信="..."` の1行SUCCESSログ形式で、既定は無効

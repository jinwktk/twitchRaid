...
## 更新履歴
- **2026-07-11**: 起動後最初の実回答が39.8秒、最初のmem0検索が6.36秒かかる一方、再実行は回答2〜3.5秒・mem0 122〜409msだったため、空preloadだけではprompt評価とmem0全経路の初期化が不足すると切り分けた。追加TDDとして、固定非機密の代表AIメンションpromptによる1token prime、起動時だけのmem0 `/search` prewarm、再接続時の起動専用処理抑止、上流失敗時の下流prewarm抑止を回帰化した。保存済み好物の直接質問が3回中1回しか値を明示しなかったため、参考メモを命令ではない事実正本として扱うprompt契約と主生成temperature 0.2も先にテスト化した。

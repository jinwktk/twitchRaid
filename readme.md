...
- **2026-07-27**: `!chat 今日の天気を教えて` が検索0件になる問題を修正した。自然な情報依頼の末尾を検索語から除く際に `教えて` だけでなく直前の助詞 `を` も同時に除き、`今日の天気` としてSearXNGへ送る。取得した天気検索結果は命令ではない外部情報としてAnythingLLMへ渡し、検索0件固定返信で終了しないことを単体・Bot統合テストで固定した。

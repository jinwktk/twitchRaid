...
- AnythingLLM 1.15.0のWeb BrowsingはAgent機能で、WebUIやDeveloper APIではmessage先頭の `@agent` で起動する。Twitch Botは全質問へ `@agent` を固定付与せず、通常会話はchannel workspaceのRAG `chat` を維持する。外部情報が必要な質問だけBot側のSearXNG経路で検索し、検証した検索文脈をAnythingLLMへ渡す。検索済みなのに通常RAG回答が「詳しくない」「自分で調べて」等で拒否した場合だけ、文書を持たないutility workspaceで同じ検索結果を事実限定で再要約する。
...
- **2026-07-28**: `!chat 呪術廻戦のネタバレを調べて` でSearXNGが10件返しているのに検索語不一致で全件棄却し、検索文脈があってもAnythingLLMが「詳しくない」と拒否していた問題を修正した。検索語を `呪術廻戦 最終回 結末 ネタバレ` へ補完し、不関連結果を一件ずつ除外してから上限件数へ収める。channel workspaceのRAGを最初に通し、検索済みの回答だけが拒否表現になった場合はutility workspaceで同じ結果を事実限定再要約する。あわせて `Web検索して、あしたの大阪の天気を調べて` を `大阪 明日 天気` へ正規化し、tenki.jpのJST翌日日付と `tomorrow-weather` 節を検証した天気・最高・最低気温だけで送信直前補正する。AnythingLLMの `@agent` はWebUI用に維持するが、通常RAGと処理時間を安定させるためBot全質問には固定付与しない。

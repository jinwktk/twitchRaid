...
- `CHAT_AI_PREWARM_ENABLED=true` にすると、Bot起動直後と `CHAT_AI_PREWARM_INTERVAL_SECONDS`（既定600秒）ごとに、AIメンション会話モデルへ `POST /api/generate` の空preloadリクエストを送り、tokenを生成せずOllamaの常駐期限を延長します。prewarmはチャット送信せず、`CHAT_AI_KEEP_ALIVE`（既定30分）を同じリクエストへ渡します。`CHAT_AI_MEM0_EMBED_PREWARM_ENABLED=true` と `CHAT_AI_MEM0_EMBED_MODEL=nomic-embed-text` も設定すると、同じcycleで埋め込みモデルへ `POST /api/embed` の固定非機密 `warmup` を送り、生成モデルと同時にロードを開始します。埋め込みAPIには公式body仕様にない `keep_alive` を渡さず、Ollama serviceの `OLLAMA_KEEP_ALIVE` で常駐を維持します。`CHAT_AI_PREWARM_TIMEOUT_MS` は既定90000で、どちらかのprewarmが失敗しても会話機能は止めずログだけ残して次回間隔で再試行します。

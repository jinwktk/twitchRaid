...
- `CHAT_AI_AUTO_LEARN_ENABLED=true` を設定すると、`覚えて: key=value`、`記憶して key=value`、`メモして key: value`、`忘れないで keyはvalue` のような明示的な記憶依頼をBot側メモへ保存します。これはモデル重みの学習ではありません。`私はカレーが好きって覚えて`、`覚えて: 私はカレーが好き`、`私は社会人だよって覚えて`、`るっかの好きなゲームはVALORANTって覚えて`、`るっかるんの口調は年相応って覚えといて` のような自然文も、暗黙メモと同じ安全抽出で `発言者の好きなもの: カレー` や `発言者: 社会人` へ正規化して保存します。`覚えといて` / `覚えといてください` / `覚えとけ` / `記憶しといて` / `メモっといて` のような口語の依頼も、明示メモとしてOllamaへ送らず処理します。主語やキーがない `43歳って覚えて` や `43歳って覚えといて` のような曖昧な依頼は従来通り形式不正で拒否します。`CHAT_AI_MEMORY_STORE=sqlite` ではSQLiteへ保存し、`CHAT_AI_MEMORY_PATH` のJSONは初回移行元としてだけ読みます。保存できるユーザーは `CHAT_AI_MEMORY_WRITER_USERS` のカンマ区切りリストで、未設定時は `rukalun` だけです。`CHAT_AI_MEMORY_WRITER_USERS=all` の場合は全ユーザーが保存できます。保存リクエストはOllamaへ送らず、成功時は固定で `覚えたD！` と返し、通常AI会話のクールダウンも消費しません。`!chat 覚えて: ...` や `!chat ...って覚えといて` のコマンド検出ログは `[memory-request]` に伏せます。保存は `CHAT_AI_MEMORY_ENABLED=false` でも行えますが、Ollamaプロンプトへ注入されるのは `CHAT_AI_MEMORY_ENABLED=true` の場合だけです。既定上限は `CHAT_AI_AUTO_LEARN_MAX_KEY_CHARS=40`、`CHAT_AI_AUTO_LEARN_MAX_VALUE_CHARS=120`、`CHAT_AI_AUTO_LEARN_MAX_ITEMS=50` で、保存時はURL、メール、電話番号、token/API key/password系、本名/住所/誕生日などの個人情報キー、予約キー `global` / `users` / `__meta`、プロンプト注入系の文面を拒否します
- `CHAT_AI_IMPLICIT_MEMORY_ENABLED=true` を追加すると、明示的に「覚えて」と言わない通常AIメンションでも、返信送信に成功した後で安全な短文だけをルールベース抽出して記憶ストアへcandidateとして観測します。対象は `私はカレーが好き`、`私は社会人だよ`、`るっかは43歳`、`るっかの好きなゲームはVALORANT` のような安定した事実・嗜好に限定し、疑問文、依頼文、今日/昨日などの一時情報、会話の打ち切りや勝ち負けなどの一時反応、なぞなぞ、Twitchエモート混じりのBot返答、秘密情報、個人情報キー、プロンプト注入文は保存しません。一人称の一般属性は発言者名へ正規化しますが、`私は43歳だよ` のような年齢・生年月日・居住地系は保存しません。暗黙保存は追加のOllama呼び出しや追加prompt生成を行わず、チャット返信後に非同期で実行します。保存権限と上限は `CHAT_AI_AUTO_LEARN_ENABLED` / `CHAT_AI_MEMORY_WRITER_USERS` / `CHAT_AI_AUTO_LEARN_MAX_*` と同じです。candidateはOllamaやmem0へ出さず、同じ事実が昇格した後だけ `kind=implicit` のactiveメモとして関連取得とmem0ミラーの対象になります
- `CHAT_AI_COMMENT_MEMORY_ENABLED=true` を追加すると、`!chat` やBot宛てメンションではない通常の配信コメントも、チャット返信やOllama呼び出しを行わず非同期で記憶候補として読みます。既定は無効です。保存するのは通常コメント本文そのものではなく、抽出済みの `key: value` だけです。対象は `私はカレーが好き`、`私は社会人だよ`、`カレーが好き` のような発言者本人の安定した自己紹介・嗜好と、`るっか` / `るっかるん` / `rukalun` / `にめいや` / `にめいやボットくん` / `nyme_ia2` など既知対象の安定事実に限定します。質問、依頼、一時的な近況、なぞなぞ、会話打ち切り、悪口、Twitchエモート混じり、URL、メール、電話番号、token/API key/password、プロンプト注入、一人称の年齢・生年月日・居住地は保存しません。保存権限とストアは `CHAT_AI_AUTO_LEARN_ENABLED` / `CHAT_AI_IMPLICIT_MEMORY_ENABLED` / `CHAT_AI_MEMORY_WRITER_USERS` / `CHAT_AI_MEMORY_STORE` に従い、candidateとして観測し、同じ事実が `CHAT_AI_MEMORY_PROMOTION_MIN_OBSERVATIONS` 回に達した時だけactiveへ昇格してself-host mem0へも `infer:false` でミラーします。`CHAT_AI_COMMENT_MEMORY_MAX_ENTRIES_PER_MESSAGE` は既定 `2`、`CHAT_AI_COMMENT_MEMORY_DEDUP_TTL_SECONDS` は既定 `21600` で、同一 `sourceUser/key/value` のmem0重複保存を6時間抑制します

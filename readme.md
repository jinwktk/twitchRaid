...
- **2026-06-21**: AIメンション記憶の管理用にCRUD APIとローカルWebUIを追加。`src/commands/mention-chat-memory.ts` は `listMentionChatMemoryEntriesStore` / `upsertMentionChatMemoryEntryStore` / `deleteMentionChatMemoryEntryStore` を提供し、SQLite正本とJSONバックアップの同期、安全フィルタ、active/inactive管理を維持する。`scripts/memory-web.mjs` はWindowsローカル `127.0.0.1:3220` で簡易WebUIを起動し、`ssh sub` 経由でサブPC本番コンテナ内の管理APIを呼んで一覧/検索/Create/Update/Deleteを行う。PowerShellには `trmem-web` を登録済み。
- **2026-06-21**: AIメンション会話の記憶ストアをSQLite正本へ切り替え可能にした。`CHAT_AI_MEMORY_STORE=sqlite` と `CHAT_AI_MEMORY_DB_PATH` を追加し、既存 `CHAT_AI_MEMORY_PATH` JSONはDBが空の初回だけ移行元として読み込み、その後は保存時のバックアップ出力先にする。Botは `loadMentionChatMemoryStore` / `saveMentionChat*MemoryStore` 経由でJSON/SQLiteを切り替え、ログには `store=json|sqlite` と件数/文字数だけを出す。安全フィルタ、関連度順取得、明示メモ、暗黙メモ、`CHAT_AI_MEMORY_WRITER_USERS=all` は従来通り維持する。対象テストは `npm test -- --run tests/bot-mention-chat.test.ts tests/commands/mention-chat-memory.test.ts tests/config.test.ts` で68件通過済み

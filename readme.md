...
## 更新履歴
- **2026-07-04**: リスナー同士の通常コメントもAIメンション会話の短期文脈へ一時保持し、直後の `!chat どう思う？` / `!chat どうかな？` など曖昧な追跡質問でだけOllama promptへ「命令ではない参考文脈」として渡すようにした。通常コメントだけではOllama呼び出しやチャット返信を行わず、`!chat 今何時？` のような新規話題には渡さない。URL/連絡先/秘密情報/プロンプト注入系の通常コメントは短期文脈にも入れず、本文は診断ログでも出さない。対象テストは `tests/bot-mention-chat.test.ts` に追加した。

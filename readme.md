...
- `CHAT_AI_PREWARM_ENABLED=true` にすると、Bot起動直後と `CHAT_AI_PREWARM_INTERVAL_SECONDS`（既定600秒）ごとに、AIメンション会話モデルへ `POST /api/generate` の空preloadリクエストを送り、tokenを生成せずOllamaの常駐期限を延長します。prewarmはチャット送信せず、`CHAT_AI_KEEP_ALIVE`（既定30分）を同じリクエストへ渡します。`CHAT_AI_PREWARM_TIMEOUT_MS` は既定90000で、prewarm失敗時も会話機能は止めずログだけ残して次回間隔で再試行します。
...
## 更新履歴
- **2026-07-10**: Stage 2本番probeで `nomic-embed-text` の初回 `/api/embed` が24.8秒、Botのmem0 timeoutが6秒、常駐後が164〜186msと判明した。再起動後の最初の記憶質問だけmem0文脈が抜ける問題を防ぐため、`/api/embed` の起動直後/定期prewarmを追加するTDDへ戻った。`tests/commands/ollama-prewarm.test.ts`、`tests/config.test.ts`、`tests/bot-periodic-recommendation.test.ts` に回帰testを先行追加し、旧実装で3ファイル41件中6件red、既存35件greenを確認した。

...
- **2026-07-27**: `!chat 今日の天気を教えて` が検索0件になる問題を修正した。自然な情報依頼の末尾を検索語から除く際に `教えて` だけでなく直前の助詞 `を` も同時に除き、`今日の天気` としてSearXNGへ送る。さらに日本語の `今日の天気` を一語の完全一致として要求していた関連度判定を天気検索だけ補正し、相対日は無視しつつ明示地域と `天気` は必須にした。取得した天気検索結果は命令ではない外部情報としてAnythingLLMへ渡し、検索0件固定返信で終了しないことと、別地域の結果を採用しないことを単体・Bot統合テストで固定した。

...
## 更新履歴
- **2026-07-18**: `!chat 森のくまさんの替え歌を調べて、うたって` が本番で `searchReason=no_result` となる問題を修正。SearXNGと検索設定は正常だったが、検索後の追加依頼まで検索語へ含めたため一致結果を捨てていた。明示的な検索指示の直後に読点区切りの追加依頼がある場合は、検索対象部分だけをSearXNGへ送るようにした。同じ本番環境で対象だけなら3件取得できることを確認し、`tests/commands/mention-chat-search.test.ts` に再現テストを追加した。

...
- Windows PowerShellの `trmem-web` または `npm run memory:web` で起動する `twitchRaid Memory` WebUIは、既存のURL/portを維持したままmem0を管理します。メインPC起動版は `http://127.0.0.1:3220/` で待ち受け、操作時に `ssh sub` 経由でSUB AI Services内の `sub-ai_mem0` コンテナへ入り、`MEM0_ADMIN_API_KEY` を外へ出さずに内部REST APIを呼びます。サブPC常駐版は `http://192.168.0.99:3220/` で、WebUI専用Swarm service `twitchraid-memory-web` が `dokploy-network` 上の `http://mem0:8888` へdirect接続します。画面ではscope `CHAT_AI_MEM0_USER_ID` / `CHAT_AI_MEM0_AGENT_ID` の一覧、semantic search、新規追加、Editによる本文更新、Deleteによるmem0 ID削除ができます。mem0にはinactive状態がないため、旧SQLite画面のactive/inactive切替は廃止しています。`npm run memory:web:direct` は `--endpoint` と `--api-key` で直接mem0 wrapperへ接続する検証用です。PowerShell登録先は `C:\Users\mlove\Documents\PowerShell\Microsoft.PowerShell_profile.ps1` です。

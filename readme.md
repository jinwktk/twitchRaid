...
- `Web検索して、あしたの大阪の天気を調べて` の先頭指示は検索対象から除き、`Web` だけを誤って検索しない。明示地域と相対日を `大阪 明日 天気` の形で送り、JST当日のJSON-LDでページと地域を確認した上で `tomorrow-weather` 節の月日がJST翌日と一致する時だけ、明日の天気・最高・最低気温を構造化予報として採用する。相対日は助詞・空白境界のある単独語だけを認識するため、`明日香村` を翌日と誤認せず、`日の出町` の地名も保持する。`今日と明日` の両日指定は単日予報へ変換せず、地域照合では `京都` と `東京都` を別地域として扱う。
...
- **2026-07-28**: `!chat 呪術廻戦のネタバレを調べて` でSearXNGが10件返しているのに検索語不一致で全件棄却し、検索文脈があってもAnythingLLMが「詳しくない」「外部検索する能力がない」と拒否していた問題を修正した。検索語を `呪術廻戦 最終回 結末 ネタバレ` へ補完し、不関連結果を一件ずつ除外してから上限件数へ収める。channel workspaceのRAGを最初に通し、検索済みの回答だけが拒否表現になった場合はutility workspaceで同じ結果を事実限定再要約する。あわせて `Web検索して、あしたの大阪の天気を調べて` を `大阪 明日 天気` へ正規化し、tenki.jpのJST翌日日付と `tomorrow-weather` 節を検証した天気・最高・最低気温だけで送信直前補正する。AnythingLLMの `@agent` はWebUI用に維持するが、通常RAGと処理時間を安定させるためBot全質問には固定付与しない。

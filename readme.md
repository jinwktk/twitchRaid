...
> LAN UI設定時は旧WebUIが残した `0.0.0.0:3220` portproxyを先に削除し、`192.168.0.99:3220` だけを作成します。Windows Firewallも送信先 `192.168.0.99`・送信元 `192.168.0.0/24` に限定します。`KeepDokployWslAlive` は起動時に3220/3221の転送先を現在のWSL IPへ更新し、設定済みmappingのlistener欠落とWSL PID 1のcritical mount不整合を60秒間隔で自己修復します。
...
- 2026-06-21 17:35 JST時点のサブPCDokploy本番は revision `1a496cdfd4dbe5b1979d742fa4bff8e093d4175d` を反映済み。`CHAT_AI_MEM0_ENABLED=true` / `CHAT_AI_MEM0_ENDPOINT=http://mem0:8888` / `CHAT_AI_MEM0_USER_ID=rukalun` をDokploy envへ永続化し、実コンテナからself-host Mem0 OSSへ保存/検索probeが成功することを確認済み。Mem0 APIキー値は記録しない
...
- `DISCORD_BOT_TOKEN` と `DISCORD_SUMMARY_CHANNEL_ID` を設定すると、Bot APIで配信開始通知、クリップURL、終了まとめを投稿し、配信開始通知メッセージからDiscordスレッドを作成します。stateに開始通知IDが無い再起動・同一タイトル再配信でも、対象チャンネルのactive/public archivedスレッドと開始通知履歴を読み、完全一致する `配信まとめ - <配信タイトル>` の最新スレッドまたは実際に送ったEmbedタイトル/Twitch URLを持つ自Bot・設定Webhookの起点メッセージを再利用します。履歴を最後まで正常取得して一致が無い場合だけ新しい開始通知とスレッドを作ります。429・権限・通信・ページ上限などで履歴の完全性を保証できない場合は新規投稿せず再試行し、終了まとめとクリップも通常Webhookへ外出しません。Bot APIが確実に拒否した403だけ `DISCORD_WEBHOOK_URL` へフォールバックし、network/5xx/不正応答など投稿成否が不明な場合は次回履歴走査でorphanを回収します。履歴走査とBot message/thread作成は15秒で中断してbackoffへ入り、thread作成失敗後の再照合GETも同じbudgetを使い、429の `retry_after` を保持します
...
- `OLLAMA_SHOUTOUT_ENABLED=true` と継承できるモデルがある場合、Raid時にOllama `POST /api/generate` で1通のRaid挨拶文を生成してチャットへ送信します。モデルは `OLLAMA_SHOUTOUT_MODEL` が最優先で、未設定時は `CHAT_AI_MODEL`、さらに `OLLAMA_MODEL` を使います。サブPC本番は3キーを `gemma4:e4b-it-qat` に統一しています。AI生成文はTwitchチャット上限に合わせて500文字以内で、Raid元の配信内容をなるべく長めに紹介する方針で生成・整形します。`OLLAMA_BASE_URL` は未設定時 `http://127.0.0.1:11434`、`OLLAMA_SHOUTOUT_TIMEOUT_MS` は未設定時 `15000`、`OLLAMA_SHOUTOUT_KEEP_ALIVE` は未設定時 `30m` です
- `CHAT_AI_ENABLED=true` と `CHAT_AI_MODEL`（未設定時は `OLLAMA_MODEL`、さらに `OLLAMA_SHOUTOUT_MODEL`）を設定すると、通常チャットで `@にめいやボットくん` や `@nyme_ia2` のようにBotへメンションされた時だけOllamaでTwitchチャット1通の返信を生成します。短く済む時は短く、説明や文脈整理が必要な時は `CHAT_AI_MAX_RESPONSE_CHARS`（既定500）以内で複数文でも具体的に返す方針です。AIメンション会話ではBotは `るっかるん` 本人として振る舞い、一人称を必ず `私` に固定して、自称としての `俺` / `僕` とそのひらがな・カタカナ表記（助詞などを全カタカナで連結した表記を含む）をsystem/promptと送信前検査の両方で禁止します。違反した直接Ollama返信は一度だけ専用promptで再生成し、AnythingLLM返信は既存utility再生成へ回し、再生成後も違反する場合は送信しません。質問中と同一の語句・作品名を返信内で明示的に引用する場合、`下僕` など別の語に含まれる場合、または `僕ちゃん、` のように敬称と句読点で明確なユーザー表示名の呼びかけになっている場合だけ該当表記を維持します。`CHAT_AI_ENABLED` 未設定時は `OLLAMA_SHOUTOUT_ENABLED=true` かつ継承できるモデルがある場合だけ互換的に有効として扱い、明示的な `CHAT_AI_ENABLED=false` または `0` は常に無効化を優先します。`CHAT_AI_BASE_URL` は未設定時 `OLLAMA_BASE_URL` または `http://127.0.0.1:11434`、`CHAT_AI_TIMEOUT_MS` は未設定時 `8000`、`CHAT_AI_TIMEOUT_FALLBACK_REPLY` は未設定時 `今ちょっとAIが混み合ってるD！`、`CHAT_AI_KEEP_ALIVE` は `30m`、`CHAT_AI_MAX_RESPONSE_CHARS` はTwitchチャット送信上限に合わせて `500`、`CHAT_AI_COOLDOWN_SECONDS` は `5` です。`CHAT_AI_BOT_ALIASES` と `CHAT_AI_IGNORED_USERS` はカンマ区切りで、未設定時は `CHAT_AI_BOT_ALIASES=にめいやボットくん,nyme_ia2`、`CHAT_AI_IGNORED_USERS=nyme_ia2` を使います。英語やスラングの意味を聞かれた時だけ、質問文に含まれる英単語を引用表記として許可します。それ以外の英語一般語や英文例は従来通り修正・拒否対象です。AIメンション会話の画像質問機能は廃止済みで、`CHAT_AI_STREAM_IMAGE_ENABLED` や `CHAT_AI_VISION_MODEL` が残っていてもTwitchライブプレビュー画像は取得せず、Ollamaへ `images` を送らず、常に `CHAT_AI_MODEL` のテキストモデルで処理します。`るっかるん` / `るっか` / `rukalun` の年齢質問は `!age` と同じBot内計算で固定返信し、居住地質問は個人情報として固定拒否するため、Ollamaや外部検索、記憶メモ注入には渡しません。Ollama timeout時は `reason=timeout` と `timeoutMs` / `elapsedMs` をログへ残し、fallback文が空でなければチャットへ送ります。`CHAT_AI_PROMPT_REPLY_LOG_ENABLED=true` を設定すると、Docker/DokployのConsoleには最終採用回答を `AI会話診断` / `質問` / `回答` の3レコードで要約し、構築済みpromptと生成replyの全文は日次ファイルだけへ保存します
- `CHAT_AI_CONTEXT_LENGTH` は既定4096、許容範囲512〜8192で、通常生成と英語修正生成の両方へOllama `options.num_ctx` として渡します。各AI依頼には同一の `requestId` を付け、コンテキスト準備ログでローカル記憶・mem0・初回検索の件数/理由/時間、Ollama性能ログで `total_duration` / `load_duration` / token数 / eval時間 / tokens-per-second / `done_reason` / HTTP実測時間を記録します。ログへ記憶本文やAPIキーは出しません。
- `CHAT_AI_PREWARM_ENABLED=true` にすると、Bot起動直後と `CHAT_AI_PREWARM_INTERVAL_SECONDS`（既定600秒）ごとに、AIメンション会話モデルへ `POST /api/generate` の空preloadリクエストを送り、tokenを生成せずOllamaの常駐期限を延長します。`CHAT_AI_PREWARM_PRIME_ENABLED=true` も設定した場合は、プロセス起動後の最初の接続時だけ、実際のAIメンション会話と同じsystem/prompt構造の固定非機密入力で1token生成し、初回実回答のprompt評価・生成初期化まで先に済ませます。Twitch再接続や定期cycleではこの重いprimeを繰り返しません。prewarmはチャット送信せず、`CHAT_AI_KEEP_ALIVE`（既定30分）を同じリクエストへ渡します。`CHAT_AI_MEM0_EMBED_PREWARM_ENABLED=true` と `CHAT_AI_MEM0_EMBED_MODEL=nomic-embed-text` も設定すると、生成モデルのprime完了後に埋め込みモデルへ `POST /api/embed` の固定非機密 `warmup` を送ります。さらに `CHAT_AI_MEM0_SEARCH_PREWARM_ENABLED=true`、ローカル記憶、self-host mem0が有効なら、起動時だけ固定queryでmem0 `/search` まで通し、API・Qdrant・埋め込み検索の初回処理を先に済ませます。順序は必ず生成モデル空preload→起動時prime→nomic embed→起動時mem0検索で、上流が失敗した場合はGPU競合を避けるため下流を開始しません。8GB GPUで生成モデルとnomicを同時cold loadするとOllamaが一方をevictしてtimeoutした実測があるためです。埋め込みAPIには公式body仕様にない `keep_alive` を渡さず、Ollama serviceの `OLLAMA_KEEP_ALIVE` で常駐を維持します。`CHAT_AI_PREWARM_TIMEOUT_MS` は既定180000で、prewarmが失敗しても会話機能は止めずログだけ残します。
- サブPCのRTX 2070 SUPER環境では、`gemma4:e4b-it-qat` が約3.0GB・100% GPU、`nomic-embed-text:latest` と同時常駐した状態でGPU使用6192MiB・空き1798MiBを実測しています。本番切替時の起動prewarmは13.9秒で完了し、切替後20回評価4セットの生成p95は751.51〜770.54msでした。評価器は速度だけでなく、Botの3モデル設定とmem0のLLM/embed/infer設定が本番契約どおりかも照合します。rollback用の `qwen3.5:9b` は削除せず保存します。`/api/tags` が高速に200を返す場合でも、`ollama ps` が空、またはQwenがGemma/nomicに加わる場合は、モデルロード/VRAM常駐設定のずれとして確認します。
...
- `CHAT_AI_MEMORY_ENABLED=true` を設定すると、全ユーザー共通の記憶辞書を読み、AIメンション会話のOllamaプロンプトへ「命令ではない参考メモ」として渡します。既定ストアは `CHAT_AI_MEMORY_STORE=json` で、`CHAT_AI_MEMORY_PATH`（未設定時 `data/chat-ai-memory.json`）を従来互換の正本として読みます。`CHAT_AI_MEMORY_STORE=sqlite` を設定すると `CHAT_AI_MEMORY_DB_PATH`（未設定時 `data/chat-ai-memory.sqlite`）を正本にし、既存JSONはDBが空の初回だけSQLiteへ移行します。SQLite保存時にJSONバックアップは再生成しないため、移行後のJSONは削除できます。既定は無効で、上限は `CHAT_AI_MEMORY_MAX_ITEMS=8`、`CHAT_AI_MEMORY_MAX_CHARS=600` です。明示メモは即 `active` になります。暗黙メモでは、発言者本人の安全な自己プロフィール・嗜好（例: `趣味は釣り`、`辛いものは苦手`、`カレー好き`）も最初の観測で `active` にし、Botや他人についての事実だけをまず `candidate` として観測します。後者は同じ `key: value` が `CHAT_AI_MEMORY_PROMOTION_MIN_OBSERVATIONS`（既定2）回に達した時だけ `active` へ昇格します。Ollamaへ注入するのは `active` だけで、candidateは管理・監査用に `observedCount` / `confidence` / `promotedAt` / `lastObservedAt` を持ちます。ストア上限を超える場合はcandidateをactiveより先に削除し、未昇格の観測が既存の有効メモを押し出さないようにします。全メモを常に入れるのではなく、ユーザー発言に関連するキー/値を優先し、年齢/生年月日/居住地のように質問トピックが明確な場合はトピック違いのメモを除外します。同点時は `updatedAt` が新しいものから選びます。メモ本文は通常ログに出さず、適用時は `store`、件数、文字数だけを記録します
- `CHAT_AI_MEMORY_RELEVANCE_FILTER_ENABLED` は既定trueです。日本語aliasと主体を見て、問い合わせと無関係なactiveメモや別ユーザーのメモを注入しません。一人称質問は発言者のTwitch loginだけを本人主体として扱い、食べ物/ゲーム/話し方/呼び方/名前を含む `<主体>の<topic>` はlocal/mem0共通resolverで分離します。ローカル正本のactiveは同一keyのmem0より常に優先し、candidate / inactive / 削除または `maxItems` 上限退避のtombstoneがあるkeyもmem0から復活させません。tombstoneはJSONの `__tombstones` またはSQLiteの `mention_chat_memory_tombstones` に保持し、同じkeyをactiveで再保存した時だけ解除します。ローカル＋mem0の結合後も全体を `CHAT_AI_MEMORY_MAX_CHARS` 内へ行単位で収めます。緊急rollback時だけfalseにすると旧ローカルrankへ戻せます。
- `CHAT_AI_MEM0_ENABLED=true` と `CHAT_AI_MEM0_ENDPOINT=http://mem0:8888` を設定すると、self-host Mem0 OSS REST APIを使う任意の補助記憶を有効化します。費用を絶対に発生させない方針のため、Mem0 Platform SDKやhosted APIは使わず、`https://api.mem0.ai` / `https://app.mem0.ai` はコード側で拒否します。サブPC本番はSUB AI Services内に `mem0` と `qdrant` を公開ポートなしで置き、`mem0` は `mem0ai==2.0.7` のOSSライブラリ、Qdrant、Ollama `nomic-embed-text:latest` だけで動きます。保存内容はBot側で安全抽出済みの `key: value` なので、mem0保存時は `infer:false` を送ってmem0側の追加LLM抽出を走らせません。通常会話時はmem0のsemantic search結果を参考メモとしてOllamaプロンプトへ混ぜますが、ローカルJSON/SQLite正本に同一 `key: value` または同一キーがあるmem0行は除外し、正本メモを優先します。明示メモは保存成功時にミラーし、暗黙メモは発言者本人プロフィールの初回 `active` 化、またはBot・他人についてのcandidateが `active` へ昇格した時だけ同じ `key: value` をself-host Mem0 OSSへミラーします。candidateのままではミラーしません。既定は無効で、`CHAT_AI_MEM0_ENDPOINT` 未設定なら接続しません。`CHAT_AI_MEM0_API_KEY` / `MEM0_API_KEY` はself-hostサーバの `X-API-Key` 用です。既定値は `CHAT_AI_MEM0_USER_ID=rukalun`、`CHAT_AI_MEM0_AGENT_ID=twitchRaid`、`CHAT_AI_MEM0_APP_ID=twitchRaid`、`CHAT_AI_MEM0_TIMEOUT_MS=1200`、`CHAT_AI_MEM0_MAX_RESULTS=3`、`CHAT_AI_MEM0_MAX_CHARS=600` です。サブPC本番では `CHAT_AI_MEM0_TIMEOUT_MS=6000` にしています。mem0のAPI失敗、timeout、endpoint未設定はfail-openで通常返信を継続し、ログには本文やAPIキーを出さず件数/文字数またはreasonだけを記録します
- mem0検索は `CHAT_AI_MEM0_MIN_SCORE`（既定0.5）をself-host wrapperへ `threshold` として渡し、Bot側でもscoreを再検証します。score欠損は `CHAT_AI_MEM0_ALLOW_MISSING_SCORE=false` の既定で拒否します。`CHAT_AI_MEM0_RECALL_GATE_ENABLED=true` の既定では、挨拶・お礼・短い相槌のように記憶が不要と断定できる発言だけmem0検索を省略し、通常質問は検索します。緊急rollbackはrecall gate=false、min score=0、missing score=trueで旧挙動へ戻せます。
- AnythingLLM管理画面 `http://192.168.0.99:3220/` はワークスペース・ベクター・文書登録の管理に使う。コメント本文、配信要約、出典付き事実を人間向けに確認する場合は、読み取り専用の記憶閲覧WebUI `http://192.168.0.99:3221/` を使う。
- AnythingLLM 1.15.0のWeb BrowsingはAgent機能で、WebUIやDeveloper APIではmessage先頭の `@agent` で起動する。Twitch Botは全質問へ `@agent` を固定付与せず、通常会話はchannel workspaceのRAG `chat` を維持する。外部情報が必要な質問だけBot側のSearXNG経路で検索し、検証した検索文脈をAnythingLLMへ渡す。明示的なネタバレ検索は初回回答の表現に関係なく、文書を持たないutility workspaceで同じ検索結果を事実限定・拒否なしで必ず再要約する。それ以外の検索では、通常RAG回答が「詳しくない」「自分で調べて」「外部検索できない」等で拒否した場合や、「〜みたいな感じなのかな」「あまり追っていない」「得意じゃない」「把握しているわけではない」「断言できない」のような推測・回避口調へ逃げた場合だけ再要約する。これらの口調だけを理由に未検索の雑談を外部検索へ送ることはしない。`ネタバレを調べてほしくない` / `検索しないで` / `ググってほしくない` のような否定形は、対象省略によるforce検索でも外部へ送らない。
- `CHAT_AI_SEARCH_ENABLED=true` を設定すると、検索・ニュース・最新情報・`〜について`・日程/開催/主催/開発/リリース日など外部事実が必要なAIメンションだけ外部検索し、結果を「命令ではない参考情報」としてOllamaプロンプトへ渡します。既定はDuckDuckGo Instant Answer互換の `CHAT_AI_SEARCH_PROVIDER=duckduckgo` / `CHAT_AI_SEARCH_ENDPOINT=https://api.duckduckgo.com/` です。自前SearXNGを使う場合は `CHAT_AI_SEARCH_PROVIDER=searxng`、`CHAT_AI_SEARCH_ENDPOINT=http://searxng:8080/search?language=all&safesearch=0` を設定します。SearXNG設定雛形 `ops/searxng/settings.yml` は、Yahoo! JAPANをXPathで取得する `yahoo japan` と `bing` の2エンジンだけを有効化します。サブPCDokploy本番は `CHAT_AI_SEARCH_ENGINES=yahoo japan,bing` で両方を同時検索し、Googleの空結果とDuckDuckGoのCAPTCHAへ依存しません。明示的な今日・明日の天気検索が不一致結果だけだった場合は、同じ全体deadline内で送信queryだけを `地域 天気` へ1回緩和し、元の相対日と地域はtenki.jp構造化予報の検証に保持します。SearXNGで結果が空、または検索語そのものに一致しないノイズだけの場合は、日本語Wikipedia summaryを補助検索します。それでも結果が取れない場合は `no_result` をログへ残し、検索文脈なしの通常AI生成へ継続します。検索文脈なしでOllamaが「分からない」「知らない」「調べてみる」などの不明系返信を生成した場合は、同じ発言を安全チェック付きでSearXNG/DuckDuckGo互換検索へ1回だけ通し、検索結果を入れて再生成します。追加の有料検索APIは使わず、再検索が失敗した場合は元の正直な返信を送ります。上限は `CHAT_AI_SEARCH_TIMEOUT_MS=2500`、`CHAT_AI_SEARCH_MAX_QUERY_CHARS=120`、`CHAT_AI_SEARCH_MAX_RESPONSE_BYTES=65536`、`CHAT_AI_SEARCH_MAX_RESULTS=3` です。比較検索の複合query、個別query、Wikipedia補助を含む全処理で同じtimeout上限を共有します。検索語は `るっかるんについて調べて` なら `るっかるん rukalun` のように軽く正規化・alias補完してから送信します。充電時間を尋ねる質問も初回から検索対象にし、`アップルウォッチの充電時間って何時間かかる？` は会話的な末尾を除いて `Apple Watch 充電時間` へ正規化します。`地震の最新情報を教えて` のような自然な最新情報依頼は `地震 最新` へ正規化し、対象語と `最新` の両方があるSearXNG結果だけを採用します。`!chat タン塩？塩タン？` に続く `!chat 調べて？` や `!chat どっちが正しいのか調べてほしい` のような対象省略検索では、同じ依頼者による安全な直前のAI会話発言だけを検索対象へ使い、会話履歴と検索結果の両方を回答promptへ渡します。別の視聴者の通常コメントは検索語へ使いません。短い `A？B？` 形式は比較意図を補って `A B 違い` として検索しますが、`発売日はいつ？価格はいくら？` のような別々の情報質問には追加しません。複合比較検索に一致する結果がない場合は両対象を個別に並行検索し、両方の安全な検索結果が揃った時だけ結合して回答promptへ渡します。正常応答で片方でも適切な結果がなくても通常生成へ進み、通信失敗も `failed` として通常生成へfail-openします。`森のくまさんの替え歌を調べて、うたって` のように検索指示の後へ別の依頼が続く場合は、読点より後を含めず、検索対象を `森のくまさん 替え歌` として送ります。`歌って` だけでなく `曲を作って` / `歌詞を書いて` も歌唱依頼として扱い、既存歌詞を推測・転載しない短いオリジナル歌詞を `【歌】` から始めて返します。生成結果が説明や追加質問で終わり `【歌】` から始まらない場合は成功扱いせず、歌詞専用promptで1回だけ再生成します。URL、メール、電話番号、token/API key/password系を含む検索語は送信しません
- 初回のmem0検索と外部検索は並行開始します。検索候補の通信失敗は `failed`、正常応答だが適切な結果がない場合は `no_result` として記録します。どちらも検索文脈なしの通常AI生成へ進みます。SearXNGの不一致ノイズをWikipediaで補完できなかった場合も、元のノイズへ戻さず `no_result` にします。
- `CHAT_AI_AUTO_LEARN_ENABLED=true` を設定すると、「覚えて」「記憶して」「メモして」「忘れないで」を含む自然な記憶依頼をBot側メモへ保存します。これはモデル重みの学習ではありません。`私はカレーが好きって覚えて`、`趣味は釣りって覚えといて`、`辛いものは苦手ってメモして`、`るっかの好きなゲームはVALORANTって覚えて` のような自然文を安全抽出して正規化します。`覚えて: key=value` や `メモして key: value` も互換形式として使えますが必須ではありません。`覚えといてください` / `覚えとけ` / `記憶しといて` / `メモっといて` のような口語の依頼も、明示メモとしてOllamaへ送らず処理します。誰の何か判断できない `43歳って覚えて` のような曖昧な依頼では、key=value形式を要求せず、誰のどんなことか自然な文章で教えてもらう固定返信を返します。`CHAT_AI_MEMORY_STORE=sqlite` ではSQLiteへ保存し、`CHAT_AI_MEMORY_PATH` のJSONは初回移行元としてだけ読みます。保存できるユーザーは `CHAT_AI_MEMORY_WRITER_USERS` のカンマ区切りリストで、未設定時は `rukalun` だけです。`CHAT_AI_MEMORY_WRITER_USERS=all` の場合は全ユーザーが保存できます。保存リクエストはOllamaへ送らず、成功時は固定で `覚えたD！` と返し、通常AI会話のクールダウンも消費しません。`!chat 覚えて: ...` や `!chat ...って覚えといて` のコマンド検出ログは `[memory-request]` に伏せます。保存は `CHAT_AI_MEMORY_ENABLED=false` でも行えますが、Ollamaプロンプトへ注入されるのは `CHAT_AI_MEMORY_ENABLED=true` の場合だけです。既定上限は `CHAT_AI_AUTO_LEARN_MAX_KEY_CHARS=40`、`CHAT_AI_AUTO_LEARN_MAX_VALUE_CHARS=120`、`CHAT_AI_AUTO_LEARN_MAX_ITEMS=50` で、保存時はURL、メール、電話番号、token/API key/password系、本名/住所/誕生日などの個人情報キー、予約キー `global` / `users` / `__meta`、プロンプト注入系の文面を拒否します
- `CHAT_AI_IMPLICIT_MEMORY_ENABLED=true` を追加すると、明示的に「覚えて」と言わない通常AIメンションでも、返信送信に成功した後で安全な短文だけをルールベース抽出します。`私はカレーが好き`、`私は社会人だよ`、`趣味は釣り`、`辛いものは苦手`、`カレー好きなんだよね` のような発言者本人の安定した自己プロフィール・嗜好は、最初の観測で `active` にします。`るっかは43歳`、`るっかの好きなゲームはVALORANT` のようなBotや他人についての事実はcandidateとして観測し、既定2回の再観測後だけactiveへ昇格します。疑問文、依頼文、今日/今週/最近/今だけなどの一時情報、否定・伝聞を含む文、`これ好き` のような指示語だけの嗜好、第三者の発言に見える曖昧な好悪文、会話の打ち切りや勝ち負けなどの一時反応、なぞなぞ、Twitchエモート混じりのBot返答、秘密情報、個人情報、プロンプト注入文は保存しません。一人称の一般属性は発言者名へ正規化しますが、`私は43歳だよ` や `私は大阪在住` のような年齢・生年月日・居住地系は保存しません。暗黙保存は追加のOllama呼び出しや追加prompt生成を行わず、チャット返信後に非同期で実行します。保存権限と上限は `CHAT_AI_AUTO_LEARN_ENABLED` / `CHAT_AI_MEMORY_WRITER_USERS` / `CHAT_AI_AUTO_LEARN_MAX_*` と同じです。candidateはOllamaやmem0へ出さず、activeになったメモだけを関連取得とmem0ミラーの対象にします
- `CHAT_AI_COMMENT_MEMORY_ENABLED=true` を追加すると、`!chat` やBot宛てメンションではない通常の配信コメントも、チャット返信やOllama呼び出しを行わず非同期で読みます。既定は無効です。保存するのは通常コメント本文そのものではなく、抽出済みの `key: value` だけです。対象は `私はカレーが好き`、`趣味は釣り`、`辛いものは苦手`、`カレー好き` のような発言者本人の安定した自己紹介・嗜好と、`るっか` / `るっかるん` / `rukalun` / `にめいや` / `にめいやボットくん` / `nyme_ia2` など既知対象の安定事実に限定します。本人プロフィールは最初の観測でactiveにし、既知対象の事実はcandidateとして同じ事実が `CHAT_AI_MEMORY_PROMOTION_MIN_OBSERVATIONS` 回に達した時だけactiveへ昇格します。質問、依頼、一時的な近況、否定・伝聞、なぞなぞ、会話打ち切り、悪口、Twitchエモート混じり、URL、メール、電話番号、token/API key/password、プロンプト注入、一人称の年齢・生年月日・居住地は保存しません。保存権限とストアは `CHAT_AI_AUTO_LEARN_ENABLED` / `CHAT_AI_IMPLICIT_MEMORY_ENABLED` / `CHAT_AI_MEMORY_WRITER_USERS` / `CHAT_AI_MEMORY_STORE` に従います。active化した時だけself-host mem0へも `infer:false` でミラーします。`CHAT_AI_COMMENT_MEMORY_MAX_ENTRIES_PER_MESSAGE` は既定 `2`、`CHAT_AI_COMMENT_MEMORY_DEDUP_TTL_SECONDS` は既定 `21600` で、同一 `sourceUser/key/value` のmem0重複保存を6時間抑制します
- thinking対応モデルでも短文Bot用途で空応答にならないよう、通常チャットAIとRaid挨拶文のOllama生成リクエストにはトップレベル `think:false` を付けます。サブPC本番では `OLLAMA_MODEL=gemma4:e4b-it-qat` を共通モデルとして設定し、`CHAT_AI_MODEL` と `OLLAMA_SHOUTOUT_MODEL` も同じ値へ揃えます
...
| `!pvp` | 今日と明日のフロントラインルールを表示 | 2026-04-29 JST基準の8日周期を毎日0:00 JSTに切替。返信は `今日のフロントライン：正式ルール名 / 明日：正式ルール名`。ローテーションは[FFXIV フロントライン カレンダー](https://ffxiv-frontline-calendar.tuyurukai.info/)を参照 |
...
| `!menu` | 今日のおすすめメニューをランダム表示 | 170種類からランダム |
| `!chat <メッセージ>` | Bot宛てメンションなしでAIメンション会話と同じ返信を生成 | 既存AI会話と同じクールダウン、キュー、短期会話履歴、ローカル記憶メモ、任意mem0補助記憶、外部検索、スタンプ付与を使用 |
...
- `npm run memory:web` は全コメント台帳と配信知識DBをSQLiteのread-only接続で開き、コメント、配信要約、出典付き事実、AnythingLLM取込件数を検索・閲覧する。編集・削除APIは持たない。本番は専用`memory-web` serviceをWSL host port 3221へ公開し、Windows portproxy/Firewallで`192.168.0.0/24`から`http://192.168.0.99:3221/`だけに制限する
...
- 本番AI会話はBotが構築した安全ラベル付きpromptをAnythingLLMのstable channel sessionへ送り、AnythingLLMがOllamaを呼ぶ。旧SQLite/mem0は参照せず、`覚えて`を含む発言も別ストアへ横取りせず全コメント台帳とAnythingLLMへ1回だけ保存する。英語一般語が残る候補や歌唱契約違反は専用utility sessionで1回だけ修正し、なお不正なら送信しない。AnythingLLM障害時は直接Ollamaへ黙って迂回せず、既存の短い失敗返信を返す
- `CHAT_AI_PROMPT_REPLY_LOG_ENABLED=true`では、AnythingLLM経路もrequest ID付きで追跡する。Dokployコンソールには元質問と最終返信だけを`AI会話診断`、`質問`、`回答`の3行で表示し、実際にAnythingLLMへ送った構築済みpromptと最初の生回答全文は日次ログへ`fileOnly`で保存する
...
- Bot自身や `CHAT_AI_IGNORED_USERS` の発言には返信しない。除外ユーザーのBot宛てメンションは `AIメンション会話をスキップ: ignored_user=...` とINFOログへ残る。Ollama処理中、キュー処理中、クールダウン中の追加メンションはチャットへ順番待ち通知を出さず、内部キューへ積んで待ち時間を挟み順番に生成する。返信生成を試みた時点から `CHAT_AI_COOLDOWN_SECONDS` のクールダウンをかける。Ollama失敗、HTTPエラー、空応答、無効応答ではチャットへ何も返さず、`reason=http_error` / `invalid_response` / `policy_rejected` / `exception` のようにログだけ残す。timeoutだけは `reason=timeout` として `model` / `image` / ユーザー入力prompt / `timeoutMs` / `elapsedMs` / `error` を短縮ログへ残し、`CHAT_AI_TIMEOUT_FALLBACK_REPLY` が空でなければその文をチャットへ送る。HTTPエラー時は `status` / `model` / `image` / ユーザー入力prompt / `elapsedMs` / `detail` を短縮して出す。`detail` は4KBまで読み、過大本文は `too_large`、読取失敗は `unavailable` とし、Bearer tokenやpassword/API key系の値はマスクする。送信するAI返信は通常 `AIメンション会話応答: user=... model=... image=false, prompt="..." reply="..."` としてINFOログへ短縮出力し、`policy_rejected` ではOllamaのraw返却値も短くWARNログへ残す。全文診断が有効なOllama生成経路では、直前の診断と重複しないよう最終INFOを `requestId` / `user` / `alias` / `model` / `source` / `image` / `replyChars` だけにする。固定返信と検索0件返信は診断を生成しないため、全文診断が有効でも従来どおりprompt/reply本文をINFOへ残す
- `CHAT_AI_PROMPT_REPLY_LOG_ENABLED=true` の場合、Docker/DokployのConsoleには成功時 `AI会話診断: requestId=..., result=success, context=...`、`質問: ...`、`回答: ...` の3レコードだけをSUCCESSで出す。`context` は本文ではなく `memory` / `history` / `search` の有無だけを示す。不明系回答から強制検索・再生成した場合も、途中の未採用回答は表示せず、最終的に採用した回答だけをこの3レコードへ出す。失敗時も同じ診断ヘッダと質問、存在する場合だけフォールバック/詳細をINFOで要約する。実際にOllamaへ送った構築済みprompt、生成reply、fallback、detailの全文は `logs/bot_YYYY-MM-DD.log`（コンテナでは `/app/logs/`）だけへ `AIメンション会話プロンプト/Success:` / `AIメンション会話プロンプト/失敗:` として、従来どおり220文字単位の独立レコードで保存する。この全文には参考メモ、mem0メモ、外部検索文脈、直近会話本文が含まれるため、必要な調査時だけ有効化する
...
- `作品名のネタバレを調べて` は、作品名に含まれる `の` を壊さず、検索修飾語直前の `の` だけを区切る。話数指定のない依頼は `作品名 最終回 結末 ネタバレ` へ補い、タイトル・本文に作品名と `ネタバレ` / `最終回` / `結末` のいずれかがある結果だけを一件ずつ検証し、上限件数へ収める。明示的なネタバレ検索でSearXNGの初回応答に一致結果がない場合は、同じ全体deadline内で同一検索語を1回だけ再試行する。`作品名のネタバレして` も同じ経路へ送り、`ネタバレしてほしくない` のような否定文は検索候補にしない。
- `Web検索して、あしたの大阪の天気を調べて` の先頭指示は検索対象から除き、`Web` だけを誤って検索しない。明示地域と相対日を `大阪 明日 天気` の形で送り、SearXNGの初回応答に地域と天気が一致する結果がない場合は、同じ全体deadline内で同一検索語を1回だけ再試行して検索結果の一時的な揺れを吸収する。JST当日のJSON-LDでページと地域を確認した上で `tomorrow-weather` 節の月日がJST翌日と一致する時だけ、明日の天気・最高・最低気温を構造化予報として採用する。相対日は助詞・空白境界のある単独語だけを認識するため、`明日香村` を翌日と誤認せず、`日の出町` の地名も保持する。`今日と明日` の両日指定は単日予報へ変換せず、地域照合では `京都` と `東京都` を別地域として扱う。
...
- Bot側の記憶機能として、`CHAT_AI_MEMORY_ENABLED=true` の場合だけ共通メモをプロンプトへ入れる。これはモデル重みの学習ではなく、返信ごとの参考メモ注入である。`CHAT_AI_MEMORY_STORE=json` では `CHAT_AI_MEMORY_PATH` のルート直下キー値を従来互換の正本として使う。`CHAT_AI_MEMORY_STORE=sqlite` では `CHAT_AI_MEMORY_DB_PATH` の `mention_chat_memory` テーブルを正本にし、既存JSONはDBが空の初回だけ移行元として読む。SQLite保存時にJSONバックアップは再出力しないため、取り込み確認後のJSONは削除してよい。JSONの `users.<Twitchログイン名>` は使わず、旧 `global` 配列だけは移行用に共通メモとして読める。読み込み時は `status=active` のメモだけを候補にし、質問文に関連するキー一致、キー/値の語句一致、更新日時の順で上限内へ絞る。年齢質問には年齢/生年月日メモを許可するが、居住地質問に生年月日メモを入れるようなトピック違いは除外する。`CHAT_AI_AUTO_LEARN_ENABLED=true` の場合は明示的な「覚えて/記憶して/メモして/忘れないで」依頼を自然文から抽出し、`key=value` は任意の互換形式として扱う。`CHAT_AI_MEMORY_WRITER_USERS` に含まれるユーザーだけが保存できる。`CHAT_AI_IMPLICIT_MEMORY_ENABLED=true` の場合は、通常AI返信送信後に追加Ollama呼び出しなしで短い安定事実・嗜好だけを暗黙保存し、発言者本人の安全なプロフィールは初回active、Botや他人についての事実はcandidateから既定2観測でactiveにする。`CHAT_AI_MEMORY_WRITER_USERS=all` は全ユーザー許可として扱う。保存リクエストはOllamaへ送らず、保存成功/形式不正/安全拒否/権限拒否/無効を固定文で返し、通常AI会話のクールダウンは消費しない。保存ログやAI応答ログにはメモ本文を出さず、コマンド検出ログでもメモ依頼本文を `[memory-request]` に伏せる。秘密情報、トークン、個人情報、プロンプト注入文はメモに書かない。手動編集や旧形式に混入した危険メモも読み込み時に除外する
- 通常の配信コメントからの記憶抽出は `CHAT_AI_COMMENT_MEMORY_ENABLED=true` の場合だけ動く。非コマンド・非Botメンションのコメントを対象にし、Ollama生成、チャット送信、AI会話クールダウン消費は行わない。保存範囲は発言者本人の安定プロフィール・嗜好と既知対象の安定事実に限定し、本人プロフィールは初回active、既知対象の事実は設定された観測回数でactiveにする。全文ログ保存や曖昧な会話反応の保存はしない。1コメントからの保存上限は `CHAT_AI_COMMENT_MEMORY_MAX_ENTRIES_PER_MESSAGE`、同一 `sourceUser/key/value` のmem0重複保存抑制は `CHAT_AI_COMMENT_MEMORY_DEDUP_TTL_SECONDS` で調整する
- Bot改善要望メモはAI会話メモとは別機能で、`BOT_REQUEST_NOTES_ENABLED=true` の場合だけ、非コマンド通常コメント、Bot宛てメンション、`!chat` から「Bot改善・機能追加・不具合対応として求められていそうな発言」を決定的ルールで抽出し、`BOT_REQUEST_NOTES_DB_PATH`（既定 `data/bot-request-notes.sqlite`）へ保存する。保存対象は `pending / planned / done / rejected / duplicate` の独立キューで、既存の `mention_chat_memory` やmem0、Ollama promptには入れない。保存時にチャット返信、Ollama呼び出し、AI会話クールダウン消費は行わない。URL、メール、電話番号、token/API key/password、プロンプト注入、通常雑談、検索依頼、記憶依頼、単発質問は保存しない。`BOT_REQUEST_NOTES_DIGEST_ENABLED=true` の場合、keep-alive内で未対応要望を `BOT_REQUEST_NOTES_DIGEST_INTERVAL_HOURS`（既定168時間）ごとに `BOT_REQUEST_NOTES_DIGEST_FILE_PATH`（既定 `data/bot-request-notes-digest.md`）へMarkdownで書き出す。Discordへ本文を出したい場合だけ `BOT_REQUEST_NOTES_DIGEST_DISCORD_ENABLED=true` を設定し、送信先は `BOT_REQUEST_NOTES_DISCORD_CHANNEL_ID`、未指定なら `DISCORD_SUMMARY_CHANNEL_ID`、Bot APIが使えない場合やBot API投稿が403などで失敗した場合は既存Webhookへfallbackする。ファイル保存成功後、Discord通知を有効にしている場合はDiscord送信成功後だけ送信済み時刻を記録するため、ファイル保存失敗や有効化したDiscord通知失敗時は次回tickで再試行する。確認・status更新用に `npm run request-notes:web` で `twitchRaid Bot Request Notes` WebUIを起動できる
- SUB AI ServicesのOllamaモデルは `/home/mlove/dokploy/ollama`、Whisper/SBVITS2共有Hugging Face cacheは `/home/mlove/dokploy/huggingface` のWSL ext4領域へ置く。切替時は従来の `/mnt/c/Users/mlove/.ollama` と `/mnt/c/Users/mlove/.cache/huggingface` から先行コピーし、ファイル数と総バイト数を照合してからservice mountを変更する。旧ディレクトリは安定確認まで削除せず、rollback時はmountを旧pathへ戻して対象serviceを更新する。Ollamaは `OLLAMA_NUM_PARALLEL=1`、context 4096を維持し、Gemmaとnomicを保持する `OLLAMA_MAX_LOADED_MODELS=2` とFlash Attentionを明示する
- Windows Scheduled Task `KeepDokployWslAlive` は、追跡元 `ops/sub-ai-services/keep-wsl-dokploy-alive.ps1` と `ops/sub-ai-services/refresh-wsl-dokploy-portproxy.ps1` を、それぞれ `E:\GitHub\BotManager\scripts\keep_wsl_dokploy_alive.ps1` と `E:\GitHub\BotManager\scripts\refresh-wsl-dokploy-portproxy.ps1` へ配置して使う。起動時は80/443/3000/5000/8888/11434と3220/3221を現在のWSL IPへ更新し、全mappingとFirewallをWindows LAN IP `192.168.0.99`・送信元 `192.168.0.0/24` へ限定する。稼働中は60秒ごとに3220/3221/設定済み3222のWindows listenerを確認し、mappingがあるのにlistenerだけ欠けた時に限りIP Helperを1回Start/Restartして再検査する。加えて通常shellとsystemd PID 1の両方から `/mnt/e/GitHub/RukalunPage` を `stat` し、通常shellは成功してPID 1だけ厳密に `Invalid argument` となる不整合が2回連続した場合だけ、`E:\GitHub\BotManager\logs\dokploy-wsl-mount-recovery-state.json` へ試行時刻を保存してから `wsl.exe --terminate Ubuntu-Backup` を1回実行する。PID 1 probeは入れ子のquoteを含むためUTF-8/base64化したwrapperだけをPowerShell 5.1から `bash -lc` へ渡す。この操作では同distribution上の全Docker/Swarm/SUB AI serviceが一時停止する。terminate後は10秒待ち、次loopでportproxy更新、WSL起動、Docker起動を行い、正常probe後だけ `recovery result=success` を記録する。試行後15分はScheduled Taskを再起動しても再terminateしない。mount元自体が読めない場合やPID 1 probeが別理由で失敗した場合は無限再起動を避けて60秒待機する。state保存失敗、アプリHTTP失敗、mapping欠落でもdistributionは再起動しない。反映時は両配置先SHA-256一致、TaskのStop→Start、`WSL mount namespace recovery result=success`、最新 `enter WSL keepalive`、Task State=`Running`、Bot service `1/1`、起動後のTwitchログイン、`npm run verify:anythingllm-lan-ui` の `online` を確認する
- `npm run perf:sub-ai-services` は実チャット送信・mem0書込みなしで、Scheduled Taskと配置script hash、実効BotのAI/mem0/SearXNG設定、6サービス1/1、計測中task不変・restart/error 0、固定生成、768次元embed、score閾値付きmem0、SearXNG結果、各p95上限を検証する。20回の直列生成と外部検索を行うため配信外のmaintenance時に実行する。実行にはSSH alias `sub` と稼働中のSUB AI Servicesが必要
...
## 配信通知仕様
- 配信開始検知時に Discord Bot API で通知（Bot設定が無い場合はWebhookを使用し、Bot APIが確定403で拒否した場合だけWebhookへフォールバック。network/5xx/不正応答は二重投稿を避けるため履歴復旧へ回す）
...
- 配信開始時にBot API投稿ができなかった場合、開始通知は通常Webhook投稿へフォールバックし、返されたWebhook message IDを先に保存してからBot Tokenでそのメッセージをスレッド化する。終了まとめとクリップURLは開始通知スレッドがある場合だけ投稿し、Bot API失敗時も `thread_id` 付きWebhookで同じスレッドへフォールバックする。開始通知スレッドを保証できない場合は通常Webhookへ外出しせず、pending stateを残して次回起動/監視で再試行する
- process内single-flightを本番の重複防止境界として使うため、Swarm serviceはreplica 1かつ更新順 `stop-first` を必須とする。`start-first` の旧新process overlapを許可しない
...
- レイド検知時は `src/commands/raid-info.ts` でレイド元の配信情報を取得し、Ollamaが無効または失敗した場合はチャットへ `レイドありがとうD！！ @ユーザー さんは、「ゲーム名」で「配信タイトル」をしてたD！お疲れ様D！チャンネルはこD→URL` を1通だけ送信する。実運用ではTwitch client id/access tokenを使ってHelix streams APIを `Accept-Encoding: identity` 付きで直fetchし、`Premature close` などの一時通信エラーだけ既定2回再試行する。Twurple `streams.getStreamByUserName` は認証情報が渡されないテスト/互換用fallbackとして残す
- `OLLAMA_SHOUTOUT_ENABLED=true` の場合は、取得したRaid元のユーザー名/ゲーム/タイトル/URLをもとに `src/commands/shoutout-introduction.ts` で同じ役割のRaid挨拶文を生成し、固定文の代わりに1通だけ送信する。Raid人数はAI入力に含めず、人数の多い少ないには触れさせない。モデルは明示 `OLLAMA_SHOUTOUT_MODEL`、未設定時は `CHAT_AI_MODEL`、さらに `OLLAMA_MODEL` の順に解決する
- Ollama挨拶文生成はshoutoutキュー投入後に実行する。AI生成文は `@ユーザー名` とチャンネルURLを必ず含むよう補正し、絵文字を除去し、URLを残したまま500文字以内へ丸める。プロンプトでは500文字以内でなるべく長めに、ゲーム名・配信タイトル・歓迎・労い・見どころを自然につなぐよう促す。取得済みゲーム名や配信タイトルがAI文から抜けた場合は、固定文へ戻さずコード側で不足分だけを補ってAI文として採用する。タイトルの括弧内装飾や `@ユーザー名` まで完全一致していなくても、主要部が既に含まれていれば長い定型紹介文は追記しない。`人数少なかった`、`少人数`、`寂しい` などRaid規模を下げる表現を含む場合はAI文を採用せず、固定のRaid挨拶文へフォールバックする。Ollamaが未設定、タイムアウト、HTTPエラー、空応答、日本語かなを含まない返答の場合も固定文へフォールバックし、Twitch shoutout APIは継続する。`CHAT_REPLY_EMOTES` が設定されていれば、AI/固定フォールバックどちらのRaid挨拶にもTwitchエモートコードを末尾へ付け、最終送信本文でも500文字以内・チャンネルURL保持・Raid用エモート保持を満たす
...
│   ├── bot-request-notes.ts       # チャット由来Bot改善要望メモSQLiteキュー
...
├── bot-request-notes-web.mjs      # Bot要望メモ確認・status更新WebUI
└── anythingllm-memory-web.mjs     # AnythingLLM記憶の読み取り専用WebUI
ops/
...
- **2026-08-22**: `!manga` はDLsiteの男性向けコミック日間ランキング（`https://www.dlsite.com/maniax/ranking/day?category=comic`）と女性向けコミック日間ランキング（`https://www.dlsite.com/girls/ranking/day?category=comic`）を並列取得して候補を統合する。片方のランキング取得が失敗しても、もう片方が取得できればおすすめを返す。返信形式、ON/OFF切替、10秒後自動削除は変更しない。
...
- **2026-07-28**: SearXNG検索を単一のBing固定から、Yahoo! JAPANをXPathで取得する `yahoo japan` とBingの複数エンジン構成へ変更した。サブPC本番契約は `CHAT_AI_SEARCH_ENGINES=yahoo japan,bing` とし、実機で空結果だったGoogleとCAPTCHAになったDuckDuckGoは検索対象から外す。明示地域を含む今日・明日の天気で初回結果が不一致だった場合は、同じ全体deadline内で送信queryだけを `地域 天気` へ1回緩和し、元の相対日・地域はtenki.jpの予報日と地域境界の検証に保持する。隔離した実機probeでは複合検索で天気20件、ネタバレ17件を取得し、現行Bot関数で両方とも2回連続 `found` を確認した。
- **2026-07-28**: `!chat 呪術廻戦のネタバレを調べて` でSearXNGが10件返しているのに検索語不一致で全件棄却し、検索文脈があってもAnythingLLMが「詳しくない」「外部検索する能力がない」と拒否していた問題を修正した。検索語を `呪術廻戦 最終回 結末 ネタバレ` へ補完し、不関連結果を一件ずつ除外してから上限件数へ収める。channel workspaceのRAGを最初に通し、検索済みの回答だけが拒否表現になった場合はutility workspaceで同じ結果を事実限定再要約する。あわせて `Web検索して、あしたの大阪の天気を調べて` を `大阪 明日 天気` へ正規化し、明示的なネタバレ検索でSearXNGが一時的に不一致結果だけを返した場合は同一queryを1回だけ再試行し、天気は明示地域がある場合だけ送信queryを `地域 天気` へ緩和して再試行する。tenki.jpのJST翌日日付と `tomorrow-weather` 節を検証した天気・最高・最低気温だけで送信直前補正する。AnythingLLMの `@agent` はWebUI用に維持するが、通常RAGと処理時間を安定させるためBot全質問には固定付与しない。失敗済みの配信知識jobは自動再開を6時間間隔に制限し、5分周期の再試行が単一並列のOllamaを占有して通常チャットをタイムアウトさせないようにした。配信終了直後と明示的な個別再実行は従来どおり即時実行する。
- **2026-07-27**: `!chat 今日の天気を教えて` が検索0件、または検索後も「天気は知らない」と返る問題を修正した。自然な情報依頼の末尾を検索語から除く際に `教えて` だけでなく直前の助詞 `を` も同時に除き、`今日の天気` としてSearXNGへ送る。さらに日本語の `今日の天気` を一語の完全一致として要求していた関連度判定を天気検索だけ補正し、相対日は無視しつつ明示地域と `天気` は必須にした。検索スニペットが予報サイトの案内だけで具体値を持たない場合に備え、許可したHTTPSのtenki.jp日別ページからJSON-LDの地域、今日の予報、最高/最低気温、予報日だけを既存上限内で取得し、予報日がJST当日と一致する時だけchannel workspaceのRAGを維持したままAnythingLLMへ渡す。AnythingLLMが不明表現を返すか具体値を落とした場合だけ、送信直前に同じ構造化予報から全項目入り返信へ補正する。検索0件固定返信で終了しないこと、別地域・前日データを採用しないこと、具体的な予報値をBot経由で必ず送ることを単体・統合テストで固定した。
- **2026-07-26**: AI会話の固定ルールを毎回のuser messageへ重複させず、AnythingLLM channel workspaceの`openAiPrompt`へ移した。既存workspaceでも初回アクセス時の所有確認後に期待system promptと一致しなければDeveloper APIで同期し、応答のname/slug/promptが一致しない場合はfail-closedにする。通常メッセージはチャンネル、表示名、ログインID、発言と必要な動的文脈だけに縮小し、取得文書・配信要約を具体的な事実資料として優先する指示を固定化した。実機では通常`chat` modeが古い配信要約を優先したため、前回/直前/最近の配信まとめだけは隔離knowledge sessionの`query` modeへ切り替え、`ended_at`最大の要約を選ぶ指示を追加する。Bot側の最終出力検査は維持する。
...
- **2026-07-18**: 本番で `!chat るっかの曲作って` と `!chat 明るい感じでるっかの曲を作って` が「詳しく教えて」「リクエスト待ってる」と追加質問だけを返し、曲を作らなかった問題を修正。歌唱判定が `歌って` / `うたって` に偏って `曲作って` を認識しておらず、生成後も指示遵守を検証していなかった。`曲を作って` / `歌詞を書いて` まで判定を広げ、歌詞返信は `【歌】` 開始を必須にし、説明や聞き返しなら不合格として専用promptで1回再生成する。実Gemmaへの無送信probeは3回すべて `【歌】` から始まる歌詞だけを返した。
- **2026-07-18**: `!chat 森のくまさんの替え歌を調べて、うたって` の検索成功後に、Gemmaが動画紹介と追加質問だけを返して歌わなかった問題を修正。歌唱依頼では検索結果の要約より実行指示を優先し、既存歌詞の推測・転載はせず、検索した題材を使った短いオリジナル歌詞を追加質問なしで返すprompt契約を追加した。同じ本番Gemmaへの無送信canaryで実際に短い歌詞が生成されることを確認した。
- **2026-07-18**: `!chat 森のくまさんの替え歌を調べて、うたって` が本番で `searchReason=no_result` となる問題を修正。SearXNGと検索設定は正常だったが、検索後の追加依頼まで検索語へ含め、さらに `森のくまさんの替え歌` ではBingが「森」だけのノイズを返していた。読点以降を除外し、末尾の `の替え歌` を検索用の語区切りへ直して `森のくまさん 替え歌` を送る。同じ本番環境でこの検索語なら関連結果を取得できることを確認し、`tests/commands/mention-chat-search.test.ts` に実結果の表記差を含む再現テストを追加した。
- **2026-07-18**: 配信 `316182008817` は22:28:42 JST開始に対し、Discord開始通知が01:27:45 JST作成となり約2時間59分遅延していた。本番ログでは22:39のBot検知から約1分ごとにDiscord履歴API 429が続き、`startMessageId=none` のまま履歴走査が開始通知自体を塞いでいた。配信検知の主経路をTwitch EventSub WebSocketの `stream.online` / `stream.offline` に変更し、60秒ポーリングを接続断・購読失敗時のfallbackとして残した。EventSub用APIは配信者IDを条件にしつつ、実際のBotユーザートークンを供給・更新する認証adapterを使う。新しいstream IDを初めてstateへ保存した時だけ開始通知を履歴走査より先に送り、送信結果が不明な再試行では従来どおり厳格な履歴照合へ戻るため、即時性と二重通知防止を両立する。EventSubとポーリングが重なった状態確認はsingle-flight化し、重複イベント受信時は完了後に再確認する。対象テストは `tests/bot-stream-notification.test.ts`、`tests/streams/stream-summary.test.ts`、`tests/streams/eventsub-auth-provider.test.ts` に追加した。
- **2026-07-17**: サブPC本番のAI会話・Raid・共通Ollamaモデルを `gemma4:e4b-it-qat` へ切り替え、mem0の未使用時LLM設定も同モデルへ揃えた。Dokploy application/ComposeとSwarm serviceは変更前に `/home/mlove/dokploy/backups/twitchraid/gemma4-switch-20260717T221730Z` へ権限600で保存し、Botはupdate/rollback `stop-first`、mem0はupdate `start-first` / rollback `stop-first` を維持した。起動prewarmは13.9秒、Gemma/nomic同時常駐はGPU使用6192MiB・空き1798MiB、read-only canaryは約52〜54 tokens/s、mem0検索3件、URL完全一致、Qwen不在を確認した。一方で画面のない配信内容とRaid感想を補う既知の捏造傾向は再現したため、後続のprompt/回帰テストで抑止する。切替後20回評価4セットの生成p95は751.51〜770.54msで、全6サービス1/1、task不変、restart/error 0だった。評価器にはBotの3モデル設定とmem0のLLM/embed/infer設定をfail-closedで照合する契約も追加した。rollback用Qwenは削除せず保持する。
...
- **2026-07-11**: commit `366db8f` をサブPCへ反映し、Dokploy DBとSwarm serviceをreplica 1・update/rollback `stop-first`へ永続化した。旧container終了から新container開始まで約630ms空き、process overlapなし。未投稿stream `316151050737` は同名既存thread `1525135703984443514` を復旧し、終了まとめ `1525365421887324160` を同threadへ投稿してstate `posted` になった。Discord履歴上の追加開始通知と新規threadは0、起動後の配信まとめ警告・WARN/ERROR/FATAL・container restartも0
- **2026-07-11**: Ollama改善の最終本番revisionは `21a199e1ca1cb6fa06fb40296387a4221999f540`、imageは `localhost:5050/twitch-raid-apcz9n:21a199e`（digest `sha256:6d3fc23d2374ea72eb3e8501684a57580a50ac77b361655b5aa35da0bdf91ec2`）。Dokploy/Swarmへ `CHAT_AI_PREWARM_PRIME_ENABLED=true`、`CHAT_AI_MEM0_SEARCH_PREWARM_ENABLED=true`、`CHAT_AI_MEM0_TIMEOUT_MS=10000` を永続反映した。cold直列probeはqwen空preload 80.2秒→代表prime 35.0秒→nomic embed 18.7秒→mem0検索1.82秒で、evict/load失敗/OOM/restartは0。最終代表query反映後の通常起動は0.92秒→1.54秒→0.11秒→0.15秒、mem0実質問20回は初回206ms・p95 141ms・20/20成功、好物メモは3/3、最初の生成は2.49秒だった。修正前の最初の実生成39.8秒、mem0初回6005ms・19/20、好物1/3から改善した。00:55:26〜01:10:51 JSTの15分25秒・32サンプルは主要5サービス5/5、revision固定、qwen/nomic 2model、GPU 7891MiB、Bot/Ollamaエラー0、再起動0。実チャット送信、記憶書き込み、秘密値出力は行っていない。
- **2026-07-11**: 起動後最初の実回答が39.8秒、最初のmem0検索が6.36秒かかる一方、再実行は回答2〜3.5秒・mem0 122〜409msだったため、空preloadだけではprompt評価とmem0全経路の初期化が不足すると切り分けた。固定非機密の代表AIメンションpromptによる1token prime、起動時だけのmem0 `/search` prewarm、再接続時の起動専用処理抑止、上流失敗時の下流prewarm抑止をTDDで実装した。保存済み好物の直接質問が3回中1回しか値を明示しなかったため、参考メモを命令ではない事実正本として扱うprompt契約と主生成temperature 0.2も追加した。新契約とQA追加分は対象4ファイル97件、全体44ファイル539件、Mem0 OSS wrapper 6件、build、lint、`git diff --check` がgreen。全体回帰で旧参考メモ指示全文へ依存していたmemory上限テスト1件は、`参考メモ:` の次行から `条件:` までという本来の境界で抽出するfixtureへ補正し、同テストファイル78件も再通過した。QA差し戻しでprime HTTP 500時の下流停止、intervalの空generate→embed順、mem0 API key header、mem0 HTTP 500時のfail-open・チャット0・秘密/応答本文非ログも固定した。追加の実GPU probeで、8tokenの起動query後も14tokenの初回記憶質問だけ埋め込みが6.362秒かかることを確認したため、起動時queryを実際の14token固定代表質問 `好きな食べ物なんだっけ？` へ変更した。本番は未知のtoken形でも記憶を落とさないよう `CHAT_AI_MEM0_TIMEOUT_MS=10000` を使う。独立レビューは `APPROVE / CLEAR`。
- **2026-07-10**: Stage 3でqwen/nomicを同時cold prewarmしたところ、Ollamaがqwenの予測6.3GiBを確保するため先にnomicをevictし、qwenも90秒のclient timeoutでロード中断して `ollama ps` が0件になった。promptなし・生成tokenなしの手動直列preloadでqwen 58.8秒、続くnomic 3.7秒、2model常駐へ復旧した。回帰testをqwen完了前はembedを開始しない契約へ変更し、prewarm既定timeoutを180秒にするTDDへ戻り、修正前は対象2ファイル35件中2件red・既存33件green、直列化後は対象3ファイル41件greenを確認した。全体44ファイル530件、Mem0 OSS wrapper 6件、build、lint、`git diff --check` も再度green。
- **2026-07-10**: Stage 2本番probeで `nomic-embed-text` の初回 `/api/embed` が24.8秒、Botのmem0 timeoutが6秒、常駐後が164〜186msと判明した。再起動後の最初の記憶質問だけmem0文脈が抜ける問題を防ぐため、`CHAT_AI_MEM0_EMBED_PREWARM_ENABLED` / `CHAT_AI_MEM0_EMBED_MODEL` と起動直後/定期 `/api/embed` prewarmを追加した。固定の非機密 `warmup` だけを送り、生成prewarmと同時開始し、失敗はfail-openにする。`tests/commands/ollama-prewarm.test.ts`、`tests/config.test.ts`、`tests/bot-periodic-recommendation.test.ts` に回帰testを先行追加し、旧実装で3ファイル41件中6件red・既存35件green、実装後は41件greenを確認した。全体44ファイル530件、Mem0 OSS wrapper 6件、build、lint、`git diff --check` もgreen。
- **2026-07-10**: Ollamaまわりを記憶精度・回答精度・応答性能の3軸で改善した。関連度/主体分離、local authorityと削除/上限退避tombstone、mem0 score閾値0.5・欠損score拒否・短い挨拶等のrecall gate、local+mem0全体文字数上限、mem0/初回検索の並行取得、検索 `failed` / `no_result` 分離と同一requestの失敗検索再試行抑止、SearXNG不一致ノイズ除外、Ollama `num_ctx=4096`・成功/失敗共通request ID・本文なし性能指標、tokenを生成しないpreload、Ollama serviceのkeep-alive/parallel/context既定を実装した。一人称は現在のTwitch loginだけを本人主体として扱い、食べ物/ゲーム/話し方/呼び方/名前を含む主体resolverをlocal/mem0で共有する。TDDでは旧実装に対し対象8ファイル209件中、新契約46件のredと既存163件のgreenを確認してから実装し、レビュー差し戻し境界もred→greenで追加した。最終検証は対象8ファイル220件、Mem0 OSS wrapper 6件、全体44ファイル526件、build、lint、`git diff --check`、Stage 1 rollback probe、ビルド済みdist敵対probe、秘密値scanがすべてgreen。UltraQA Cycle 1で巨大な有限metricsが丸め時にInfinityになる境界を検出し、`Number.MAX_SAFE_INTEGER` 上限と丸め後finite検査をTDD追加してCycle 2で解消した。統合時に直した4件は、照会文と期待メモの不一致、再利用不能な`Response` mock、既存の記号除去後出力、Node Buffer pool全体を返すmockというfixture/mockの実態補正で、独立レビューもTDD契約の弱体化なしと判定した。コードレビューは2回Ralplanへ戻した後、Architect/Criticとも最終 `APPROVE / CLEAR`。本番反映結果は同日の作業メモへ追記する。
...
- **2026-07-05**: チャットや `!chat` からBot改善・機能追加・不具合対応として求められていそうな発言だけを、AI会話メモとは別の `data/bot-request-notes.sqlite` に保存するBot要望メモキューを追加した。`BOT_REQUEST_NOTES_ENABLED` は既定無効で、有効時も通常雑談、検索依頼、記憶依頼、単発質問、URL/連絡先/秘密情報/プロンプト注入系は保存しない。保存時にチャット返信、Ollama呼び出し、AI会話クールダウン消費は行わず、mem0や `mention_chat_memory` にも混ぜない。`BOT_REQUEST_NOTES_DIGEST_ENABLED` で未対応要望のDiscord定期digestを有効化でき、成功時だけ送信済み時刻を記録する。確認・status更新用に `scripts/bot-request-notes-web.mjs` と `npm run request-notes:web` を追加した。対象テストは `tests/commands/bot-request-notes.test.ts`、`tests/bot-mention-chat.test.ts`、`tests/bot-periodic-recommendation.test.ts`、`tests/scripts/bot-request-notes-web.test.ts`、`tests/config.test.ts` に追加した。
- **2026-07-05**: AIメンション会話で `英語のスラングでnutsとは何ですか？` や `big peachってどういう意味？` のような英語/スラング意味質問が `english_word_repair_failed` で無返信になる問題を修正した。英語やスラングの意味説明と判定できる質問では、質問文に含まれる英単語だけ引用表記として許可し、promptと修正promptにも「その他の英単語や英文例は使わない」条件を入れる。日本語引用符は語句引用として残すため、返信全体を包む引用符だけを剥がすよう整形も調整した。対象テストは `tests/commands/mention-chat.test.ts` に追加した。
- **2026-07-05**: `CHAT_AI_PROMPT_REPLY_LOG_ENABLED=true` の全文診断ログを、成功時だけでなくtimeout、HTTPエラー、無効応答、policy rejected、例外などの失敗時にも出すようにした。失敗時は `AIメンション会話プロンプト/失敗:` に `reason=...` と実際にOllamaへ送った構築済みprompt行を出し、timeout fallbackを返す場合は `fallback[1/N]` も併記する。対象テストは `tests/commands/mention-chat.test.ts` に追加した。
- **2026-07-04**: AIメンション会話の `qwen3.5:9b` が非常駐時に約42秒ロードへ入り、`CHAT_AI_TIMEOUT_MS=45000` 付近でtimeout fallbackになりやすい問題に対し、`CHAT_AI_PREWARM_ENABLED` を追加した。Bot起動直後と `CHAT_AI_PREWARM_INTERVAL_SECONDS`（既定600秒）ごとにOllama `POST /api/generate` を `num_predict=1` / `think:false` / `keep_alive=CHAT_AI_KEEP_ALIVE` で実行し、チャット送信なしで対象モデルの常駐期限を延長する。prewarmのtimeoutは `CHAT_AI_PREWARM_TIMEOUT_MS`（既定90000）で、失敗しても会話機能は止めず次回間隔で再試行する。対象テストは `tests/commands/ollama-prewarm.test.ts`、`tests/config.test.ts`、`tests/bot-periodic-recommendation.test.ts` に追加した。
...
- **2026-07-04**: 通常の配信コメントから安全な記憶候補だけを抽出する `CHAT_AI_COMMENT_MEMORY_ENABLED` を追加した。既定は無効で、有効時も非コマンド・非Botメンションだけを対象にし、Ollama呼び出し、チャット返信、AI会話クールダウン消費は行わない。保存するのは抽出済み `key: value` のみで、発言者本人の安定プロフィール・嗜好と既知対象の安定事実に限定する。質問、依頼、一時近況、なぞなぞ、会話打ち切り、Twitchエモート混じり、URL/連絡先/秘密情報、プロンプト注入、一人称の年齢・生年月日・居住地は拒否する。`CHAT_AI_COMMENT_MEMORY_MAX_ENTRIES_PER_MESSAGE=2` と `CHAT_AI_COMMENT_MEMORY_DEDUP_TTL_SECONDS=21600` を追加し、SQLite正本とself-host mem0ミラーの回帰は `tests/commands/mention-chat-memory.test.ts`、`tests/bot-mention-chat.test.ts`、`tests/config.test.ts` に追加した。検証は対象テスト100件、全体 `npm test` 40ファイル440件、`npm run build`、`npm run lint`、`git diff --check` が通過済み。
...
- **2026-07-02**: 本番ログで `!chat レゲエパンチについて調べて` が検索候補になっていたものの、SearXNGが `reason=no_result_or_failed` で文脈なしになり、Ollamaが `調べてみるね` と未検索のまま返していた。当時は検索結果が取れない場合にBot側の固定返信へ切り替え、SearXNGが空または検索語不一致のノイズだけを返す場合は日本語Wikipedia summaryを補助検索するようにした。この固定返信方針は2026-08-23に撤回し、現在は `no_result` を記録して通常AI生成へ進む。あわせて `ops/searxng/settings.yml` は `google` だけでなく `duckduckgo` / `bing` も有効化し、Google単独の空結果に依存しない構成へ更新した。対象テストは `tests/commands/mention-chat-search.test.ts`、`tests/bot-mention-chat.test.ts`、`tests/searxng-config.test.ts` に追加した。
...
- **2026-07-02**: Ollama Raid挨拶文を短文前提から500文字上限を活かす長めの紹介文方針へ変更した。`src/commands/shoutout-introduction.ts` はsystem/promptで「500文字以内」「なるべく長め」「詳しく紹介」を明示し、`num_predict` を増やし、AI生成文の上限を500文字へ変更した。`CHAT_REPLY_EMOTES` 有効時は、送信直前のRaid用エモート付与後もチャンネルURLと `rukkaNiceraido` を残したまま500文字以内へ再整形する。対象テストは `tests/commands/shoutout-introduction.test.ts` と `tests/bot-raid-greeting.test.ts` に追加した。
- **2026-07-02**: AIメンション会話にチャンネル単位の短期会話履歴を追加した。通常生成成功の直近会話だけをプロセス内に保持し、`どんなところがすきなの？` のような省略質問へ直前の `AとBなにがすき？` / `Bがすきだよ！` の文脈を渡す。`CHAT_AI_CONVERSATION_HISTORY_ENABLED`、`CHAT_AI_CONVERSATION_HISTORY_MAX_MESSAGES`、`CHAT_AI_CONVERSATION_HISTORY_MAX_CHARS`、`CHAT_AI_CONVERSATION_HISTORY_TTL_SECONDS` を追加し、timeout fallback、勝敗質問fallback、固定返信、コマンド実行拒否、メモ保存返信は履歴化しない。`CHAT_AI_PROMPT_REPLY_LOG_ENABLED=true` の診断ログでも短期履歴本文は出さず、件数と文字数だけを出す。
- **2026-07-02**: AIメンション会話の自然文メモで、`私は社会人だよ` / `私は社会人だよって覚えて` のような一人称プロフィール文を発言者のTwitchログイン名へ正規化して保存するようにした。`私は43歳だよ` のような一人称の年齢・生年月日・居住地系は引き続き保存しない。対象テストは `tests/commands/mention-chat-memory.test.ts` と `tests/bot-mention-chat.test.ts` に追加した。
- **2026-07-02**: サブPC本番の記憶保存設定を再確認した。実行中Botは `CHAT_AI_MEMORY_WRITER_USERS=all`、`CHAT_AI_AUTO_LEARN_ENABLED=true`、`CHAT_AI_IMPLICIT_MEMORY_ENABLED=true`、`CHAT_AI_MEM0_ENABLED=true` で、非 `rukalun` ユーザーの明示メモ保存probeはSQLite正本へ保存できた。一方で `CHAT_AI_MEM0_TIMEOUT_MS=2000` ではmem0保存がBot側timeoutで `reason=failed` になり、mem0側では後続で挿入される境界状態を再現したため、Dokploy `application.env` とSwarm serviceの両方を `CHAT_AI_MEM0_TIMEOUT_MS=6000` へ更新した。新コンテナ `7fe42bacebd8` で非 `rukalun` probeがSQLite保存、mem0保存とも `result=saved` になり、検証用キーはSQLite/mem0双方から削除済み。
- **2026-07-01**: Twitchチャットの最大文字数に合わせ、AIメンション会話の既定上限 `CHAT_AI_MAX_RESPONSE_CHARS` を `200` から `500` へ変更した。`src/config.ts` の既定値と `src/bot.ts` の保険値を共通定数に揃え、`tests/config.test.ts` と `tests/bot-mention-chat.test.ts` の既定期待値も500へ更新した。`CHAT_REPLY_EMOTES` を付ける場合も最終送信本文は500文字以内に収める。
- **2026-07-01**: AIメンション会話の生成後英語チェックで、返信相手のTwitchログイン名まで英語一般語扱いして `english_word_repair_failed` になる問題を修正した。`src/commands/mention-chat.ts` は生成返信の検査時に `userName` を許可Latinトークンとして渡し、`kanonalcさん` のような呼びかけだけでは修正生成に回さない。`tonight` / `Throat pain` / `Hello there` のような英語一般語は従来通り拒否・修正対象にする。対象テストは `tests/commands/mention-chat.test.ts` に追加した。
- **2026-07-01**: サブPC本番で `Clip検索公開JSON更新をスキップ: 公開repoに開発commitが残っています protectedCommits=53, dropEligibleCommits=38` が毎分出ていた件を確認した。`E:\GitHub\RukalunPage` の checked out `main` が `origin/main` と 91/91 で分岐し、差分は `.gitignore` / `AGENTS.md` / `clip-search-data.json` だった。旧HEAD `7fb3846e770f7487e603b52c2b8fc29acbb0a87c` を `backup/subpc-rukalunpage-before-publisher-switch-20260701-1755` に残し、作業ツリーを `publisher/main-20260701` として `origin/main` へ合わせた。17:50:24 JST に Bot が `Clip検索公開JSONをorigin/mainへpushしました` まで進み、`git cherry -v origin/main HEAD` は0件、公開JSONは `generatedAt=2026-07-01T08:50:21.320Z` / `clips=2764` になった。
- **2026-06-24**: `!chat 今日の夜ご飯はなにがいい？` でOllamaが `tonight何が食べたい？...` のように英語の一般語を混ぜた返信を返した。原因はsystem/promptには日本語だけの指示がある一方、生成後の採用処理が英単語を検査せず、過去の短文許可方針で `Hello there` なども通す設計が残っていたこと。`src/commands/mention-chat.ts` は生成返信に `tonight` / `Throat pain` / `Hello there` のような英語の一般語を検出した場合、そのまま送らず一度だけ日本語だけの返信へ修正生成し、修正後も違反する場合は `english_word_repair_failed` で送信しないようにした。`GG！` のような全大文字短文や `Apex Legends` のような固有名詞は引き続き許可する。
- **2026-06-24**: AIメンション会話のOllama promptから、常時入っていた `配信画面画像: 添付なし` と配信画面前提の条件文を削除した。配信画面画像は送らない運用のため、画像未添付説明をpromptへ毎回入れない。あわせてsystem/promptの日本語指示を強化し、固有名詞やユーザー指定表記を除いて英語の一般語・中国語・ローマ字の説明語を混ぜず、症状名や専門語も日本語へ言い換えるようにした。参考メモについては、TwitchRaid Memory WebUIが見るmem0とBotが注入するSQLite正本がズレており、WebUI側に存在しない `お` キーがSQLiteに残っていたため、サブPC本番Botの `/app/data/chat-ai-memory.sqlite` をバックアップ後に同キーだけ削除した。バックアップは `/app/data/chat-ai-memory.sqlite.bak-stale-key-20260624T090000Z`。
- **2026-06-24**: AIメンション会話でOllamaへ渡す構築済みprompt全文と生成返信を本番ログへ出すため、サブPCDokploy/Swarm本番の `CHAT_AI_PROMPT_REPLY_LOG_ENABLED` を `true` へ変更した。まずSwarm service `twitch-raid-apcz9n` に `--env-rm CHAT_AI_PROMPT_REPLY_LOG_ENABLED --env-add CHAT_AI_PROMPT_REPLY_LOG_ENABLED=true` を反映して収束を確認し、実行中container `1c00ebeaacd5` の `printenv CHAT_AI_PROMPT_REPLY_LOG_ENABLED` が `true` であることを確認した。あわせてDokploy `application.env` へ同キーを永続化し、更新前application行はサブPCWSL2 `/tmp/twitchraid-application-before-prompt-reply-log-20260624.sql` へバックアップした。現在の成功ログでは `AIメンション会話プロンプト/Success:` に続けて、`prompt[1/N]` / `reply[1/N]` が各行SUCCESSログとして出る。
- **2026-06-24**: `!chat 叙々苑のランチのねだんを調べて` が `今ちょっとAIが混み合ってるD！` へfallbackした件を、サブPC本番BotとSUB AI Servicesログで調査した。Botは17:30:12 JSTにSQLiteメモ適用、17:30:14にmem0を2秒timeoutでfail-open、17:30:15にSearXNG検索結果3件を適用し、17:30:59に `qwen2.5:7b` 生成が `timeoutMs=45000` / `elapsedMs=43920` でtimeoutした。SUB AI側ではmem0検索のOllama embedが17:30:17にHTTP 200で完了し、Qdrant queryもHTTP 200だったためmem0/qdrant停止ではない。Ollamaは同時刻から `qwen2.5:7b` のcold loadに入り、`llama-server started in 42.30 seconds` の直後にBot側timeoutへ到達していた。常駐後のBotコンテナ経由生成は約1.0秒、mem0 healthは約48ms、SearXNG検索は約448msで成功したため、直接原因は検索やClipコマンドではなく、mem0 embedding cold loadが先行したうえでQwenモデル非常駐ロードが45秒枠をほぼ使い切ったことと判断した。
...
- **2026-06-22**: `clip全期間再走査` の新実装反映後、Clip本体の再走査は完了する一方でゲーム名補完のHelix games APIが `Premature close` を出していたため、ゲーム名補完もTwitch client id/access tokenと `Accept-Encoding: identity` を使うHelix直fetchへ切り替えた。Twurple games APIは認証情報がない場合のfallbackとして残す
...
- **2026-06-21**: mem0移行後にWebUIへ出ていないSQLite記憶がBotで効いていたため、本番BotコンテナのSQLite active 4件をself-host mem0へ同期した。同期前はmem0 1件、SQLite active 4件で、既存1件をスキップし不足3件を保存した。1件は初回保存がtimeout扱いになったがmem0側では保存済みだったため、再試行でできた重複を1件削除し、最終的に `http://192.168.0.99:3220/api/memory?mode=list&limit=500` はmem0 total/activeとも4件になった。メモ本文やAPIキーはログ・文書へ残していない。
- **2026-06-21**: 既存 `twitchRaid Memory` WebUIの内部データソースをSQLite正本からself-host Mem0 OSSへ切り替えた。`scripts/memory-web.mjs` は従来通り `127.0.0.1:3220` / `/api/memory` で動き、既定では `ssh sub` 経由でSUB AI Services内の `sub-ai_mem0` コンテナへ入り、APIキーを外へ出さずに一覧、semantic search、新規追加、本文更新、削除を行う。サブPC常駐版 `http://192.168.0.99:3220/` は `twitchraid-memory-web` serviceを `dokploy-network` に接続し、`http://mem0:8888` へdirect接続する構成へ更新した。検索欄に文字が残っていても `List` は通常一覧、`Semantic Search` だけsemantic searchとして扱う。`ops/mem0-oss-server/app.py` は管理用に `PATCH /memories/{id}` と `DELETE /memories/{id}` を追加した。mem0にはinactive状態がないため、旧SQLite画面のactive/inactive切替は廃止した
- **2026-06-21**: 暗黙メモ保存をmem0移行後の運用に合わせ、ローカルJSON/SQLite保存が失敗してもself-host mem0へは独立して保存するようにした。`私はカレーが好き` のような安全な通常AIメンションは、返信後に追加Ollamaなしで抽出され、mem0へ `kind=implicit` / `infer:false` で保存される。あわせてBotログの多発WARNを確認し、WSL側 `/mnt/e` の `No such device` をEドライブ再マウントとBot service再起動で復旧した。復旧後は `/app/data` が読め、`clips.sqlite` / `chat-ai-memory.sqlite` のintegrity checkはどちらも `ok`
- **2026-06-21**: `!chat ままっかが熱なんだって！` がmem0失敗後にOllama timeout fallbackになる事象へ対応した。`src/commands/mention-chat.ts` は体調不良・発熱系の短い報告を固定のいたわり返信で返し、`src/bot.ts` はメモ読込、mem0、外部検索、Ollamaより前にこの固定返信を処理する。検索/質問系や試合など非体調の熱文脈は固定返信に吸わず、チャットコマンド実行依頼は体調固定返信より優先して拒否する。`tests/commands/mention-chat.test.ts` と `tests/bot-mention-chat.test.ts` に、Ollamaを呼ばずSQLite/mem0/searchにも進まない回帰、検索/質問境界、コマンド実行拒否優先のテストを追加し、対象68件が通過。最終検証は全体 `npm test` 40ファイル391件、`npm run build`、`npm run lint`、`python -m pytest -q` 107件、`git diff --check` が通過
...
- **2026-06-21**: Mem0補助記憶をBot本番へ有効化した。commit `1a496cdfd4dbe5b1979d742fa4bff8e093d4175d` のimageを `localhost:5050/twitch-raid-apcz9n:local` / `:1a496cd` へpushし、Dokploy application envを `/tmp/twitchraid-application-before-mem0-bot-20260621173445.sql` へバックアップ後、`CHAT_AI_MEM0_ENABLED=true`、`CHAT_AI_MEM0_ENDPOINT=http://mem0:8888`、`CHAT_AI_MEM0_USER_ID=rukalun` などを永続化した。実行中Bot container `9403e3f93884` は同revision labelで起動し、`dist/commands/mention-chat-mem0.js` のprobe保存/検索が `saved` / `found` を返すことを確認した。APIキー値は記録しない
...
- **2026-06-21**: 記憶管理WebUIをサブPCのLAN URL `http://192.168.0.99:3220/` でも見られるようにした。`scripts/memory-web.mjs` は `--target direct` に対応し、非loopback bindの認証なし起動は `--allow-unsafe-no-auth` を明示した時だけ許可する。サブPCでは専用Swarm service `twitchraid-memory-web` が同じ `/app/data` をbind mountしてSQLite正本を直接管理し、Windows portproxy `0.0.0.0:3220 -> WSL2:3220` とFirewall ruleでLANから到達できる。
- **2026-06-21**: `!myclip` のキャッシュ未準備時APIフォールバックでHelix clips APIが `Premature close` になる問題に対応。`src/commands/clip.ts` はTwitch client id/access tokenがある実運用ではTwurple paginatorを使わず、`Accept-Encoding: identity` 付きのHelix clips直fetchで最大200件をページ取得し、一時通信エラーだけ既定2回再試行する。`src/bot.ts` は `!clip` / `!myclip` のフォールバックへ現在のTwitch認証情報を渡す。`tests/commands/clip.test.ts` と `tests/bot-help.test.ts` にidentity直fetchと再試行の回帰テストを追加し、対象23件が通過
- **2026-06-21**: AIメンション記憶の管理用にCRUD APIとローカルWebUIを追加。`src/commands/mention-chat-memory.ts` は `listMentionChatMemoryEntriesStore` / `upsertMentionChatMemoryEntryStore` / `deleteMentionChatMemoryEntryStore` を提供し、SQLite正本の安全フィルタ、active/inactive管理を維持する。`scripts/memory-web.mjs` はWindowsローカル `127.0.0.1:3220` で簡易WebUIを起動し、`ssh sub` 経由でサブPC本番コンテナ内の管理APIを呼んで一覧/検索/Create/Update/Deleteを行う。PowerShellには `trmem-web` を登録済み。
- **2026-06-21**: AIメンション会話の記憶ストアをSQLite正本へ切り替え可能にした。`CHAT_AI_MEMORY_STORE=sqlite` と `CHAT_AI_MEMORY_DB_PATH` を追加し、既存 `CHAT_AI_MEMORY_PATH` JSONはDBが空の初回だけ移行元として読み込む。Botは `loadMentionChatMemoryStore` / `saveMentionChat*MemoryStore` 経由でJSON/SQLiteを切り替え、ログには `store=json|sqlite` と件数/文字数だけを出す。安全フィルタ、関連度順取得、明示メモ、暗黙メモ、`CHAT_AI_MEMORY_WRITER_USERS=all` は従来通り維持する。対象テストは `npm test -- --run tests/bot-mention-chat.test.ts tests/commands/mention-chat-memory.test.ts tests/config.test.ts` で68件通過済み
- **2026-06-21**: AIメンション会話に暗黙メモ保存を追加。`CHAT_AI_IMPLICIT_MEMORY_ENABLED=true` の時だけ、返信送信成功後に `私はカレーが好き` や `るっかは43歳` のような短い安定事実・嗜好をルールベースで抽出し、`CHAT_AI_MEMORY_PATH` へ `kind=implicit` として保存する。暗黙保存は追加のOllama呼び出しや追加prompt生成を行わず、返信後に非同期実行するため通常の返信生成時間に乗せない。保存権限は `CHAT_AI_AUTO_LEARN_ENABLED` と `CHAT_AI_MEMORY_WRITER_USERS` に従い、質問/依頼/一時情報/秘密情報/個人情報キー/プロンプト注入文は拒否する。対象テストは `npm test -- --run tests/commands/mention-chat-memory.test.ts tests/bot-mention-chat.test.ts tests/config.test.ts`、全体 `npm test` 38ファイル362件、`npm run build`、`npm run lint`、`git diff --check` で通過。サブPCDokploy本番はDBバックアップ `/tmp/twitchraid-application-before-implicit-memory-20260621.sql` を作成し、`CHAT_AI_IMPLICIT_MEMORY_ENABLED=true` と `CHAT_AI_PROMPT_REPLY_LOG_ENABLED=false` を永続化した。Docker image `localhost:5050/twitch-raid-apcz9n:local` / `:1ee7c31` をpushし、deployment `eyOVfG_jvlKFq_p01nqt4` とGitHub auto deploy `5_KHiUj3pW2eXv99fcidA` は `done`。実行中container `3196328e65f4` は revision `1ee7c31ffd74a121c8c8876bd6e02ba84456168f`、起動ログ `全てのチャンネルにログインしました`、env反映、実コンテナ一時ファイルで `kind=implicit` 保存成功を確認した
- **2026-06-21**: `CHAT_AI_MEMORY_WRITER_USERS=all` を全ユーザー許可として扱うようにした。`tests/bot-mention-chat.test.ts` に任意ユーザーの記憶保存が固定返信 `覚えたD！` で成功し、Ollamaを呼ばない回帰テストを追加した。サブPCDokploy本番の `application.env` も `CHAT_AI_MEMORY_WRITER_USERS=all` へ更新済み。最新image `29bcd4f35984935dd6731b7841c4366e50a5a264` を `localhost:5050/twitch-raid-apcz9n:local` / `:29bcd4f` としてpushし、実行中containerのrevision/env、起動ログ、非 `rukalun` ユーザーの記憶保存ログ `result=saved` を確認した
- **2026-06-21**: サブPCDokploy本番の記憶設定を確認し、`CHAT_AI_MEMORY_ENABLED=true` / `CHAT_AI_AUTO_LEARN_ENABLED=true` / `CHAT_AI_MEMORY_PATH=data/chat-ai-memory.json` / `CHAT_AI_MEMORY_MAX_ITEMS=8` / `CHAT_AI_MEMORY_MAX_CHARS=600` に加えて、`CHAT_AI_MEMORY_WRITER_USERS=rukalun` と自動保存上限 `CHAT_AI_AUTO_LEARN_MAX_KEY_CHARS=40` / `CHAT_AI_AUTO_LEARN_MAX_VALUE_CHARS=120` / `CHAT_AI_AUTO_LEARN_MAX_ITEMS=50` をDokploy `application.env` へ永続化した。最新image `faf73950024c8e42ce3cc57ff97238fda7d1f90c` を `localhost:5050/twitch-raid-apcz9n:local` / `:faf7395` としてpushし、Dokploy `deployApplication` で再デプロイ済み。実行中containerのrevision/env、起動ログ、`/app/data/chat-ai-memory.json` 読み込みを確認した
- **2026-06-21**: ローカルLLM記憶領域を再設計。`src/commands/mention-chat-memory.ts` は共通JSONメモを関連度順に絞って注入し、`__meta` へ `kind/status/sourceUser/createdAt/updatedAt` を保存する。`src/bot.ts` は記憶保存リクエストを通常会話から分離し、`CHAT_AI_MEMORY_WRITER_USERS` に含まれるユーザーだけが保存できるようにした。保存成功/形式不正/安全拒否/権限拒否/無効は固定返信で完結し、Ollamaへ記憶依頼本文を送らず、通常AI会話のクールダウンも消費しない。URL、メール、電話番号、token/API key/password系、本名/住所/誕生日などの個人情報キー、予約キー、プロンプト注入文は保存拒否し、手動混入した危険メモも読み込み時に除外する。`!chat 覚えて: ...` のコマンド検出ログは `[memory-request]` に伏せる。`ws` は8.21.0、dev経路の `esbuild` は0.28.1へ更新し、`npm audit` は0件。対象テストは `npm test -- --run tests/commands/mention-chat-memory.test.ts tests/bot-mention-chat.test.ts tests/config.test.ts`、全体 `npm test`、`npm run build`、`npm run lint`、`python -m pytest -q` で通過
...
- **2026-06-21**: SearXNGを独立stackではなく既存SUB AI Services stackの中へ移した。Dokploy Compose `sub-ai-services` の `sub-ai` stackへ `searxng` serviceを追加し、旧standalone `twitchraid-searxng` のComposeレコードは削除済み。SUB AI ServicesのDescriptionは `Ollama, Whisper, SBVITS2, and internal SearXNG for twitchRaid AI mention search` に更新した。Swarm上は `sub-ai_ollama` / `sub-ai_whisper-api` / `sub-ai_sbvits2` / `sub-ai_searxng` の4サービス構成で、Botから `http://searxng:8080/search?language=all&safesearch=0&q=...&format=json&engines=google` へHTTP 200で到達する。実行中Botの `fetchMentionChatSearchContext("OpenAIについて調べて")` はSearXNG経由で `resultCount=3` を返す。運用雛形は `ops/sub-ai-services/docker-compose.yml`、SearXNG設定雛形は `ops/searxng/settings.yml`
- **2026-06-21**: サブPCDokployで画像質問後に `!chat` が連続timeoutした件を調査。`!chat この画面は何のゲーム？` は `AIメンション会話へ配信画像を添付: bytes=62458` の後、`gemma3:4b` / `image=true` で43秒経過してtimeout。Ollamaログでは `model predicted to exceed available memory, evicting` の後に `gemma3:4b` が約31秒でロードされ、POSTは約45秒で500になった。続く通常 `!chat` は `qwen3.5:9b` の再ロードが必要になり、こちらも45秒timeoutした。原因は画像サイズそのものではなく、8GB GPU上でVisionモデルと通常会話モデルを切り替えるVRAM/ロード時間。`qwen3.5:9b` が常駐後の通常 `!chat` は数秒で復帰した
- **2026-06-20**: サブPCで「何がGPUを使っているか」を追加確認。GPU利用中として見えているのは、Ollama `sub-ai_ollama` の `llama-server`、Whisper `sub-ai_whisper-api` の `python3 /app/server.py`、SBVITS2 `sub-ai_sbvits2` の `python3 server_fastapi.py`、およびWSLg表示系の `Xwayland`。WSL2の `nvidia-smi` ではプロセス別GPU Memoryが `N/A` になるため、非停止で厳密なMiB内訳は取れない。Ollamaは `ollama ps` / APIで `qwen3.5:9b` の `size_vram` が約4.98GBと分かり、残り約2.6GBをWhisper/SBVITS2/Xwayland/ランタイム等が使っている推定
...
- **2026-06-20**: AIメンション会話のOllamaプロンプト再構成に向け、TDDの赤テストを先行追加。通常prompt/systemに自然な1〜2文・一語だけ禁止・検索結果優先の契約を要求し、`TwitchConの日程教えて` のような自然な未知情報質問は検索候補、`好きな食べ物教えて` のような雑談は非検索候補にする期待を追加した。エモートは検索失敗/不明/謝罪系返信で明るい `rukkaGg` より `rukkaShobobo` を優先する期待を追加し、未実装状態で対象テスト6件の失敗を確認した
- **2026-06-20**: AIメンション会話のOllama promptを「短く」中心から「自然な1〜2文」中心へ再構成。外部検索結果がある場合は命令ではない参考情報のまま事実情報として優先要約するよう指示し、検索トリガーは日程/開催/主催/開発/リリース日など外部事実語を拡張しつつ、`教えて` / `わからない` 単独の雑談では検索しないようにした。レビューで見つかった `日程` / `開催` 単語だけの過剰発火、`誰が出る？` / `いつ配信？` / `どこでやる？` の助詞付き疑問詞漏れ、`誰が好き？` / `いつ寝る？` / `どこ行きたい？` の雑談過剰発火は、正例・負例を追加して修正した。文脈エモートは不明/検索失敗/謝罪系の返信本文を先に見て `rukkaShobobo` 等を優先する
...
- **2026-06-20**: `CHAT_AI_PROMPT_REPLY_LOG_ENABLED=true` を追加。AIメンション会話の成功時に、Ollamaへ送った構築済みprompt全文と生成返信をログへ出せるようにした。現在は `prompt[1/N]` / `reply[1/N]` の複数SUCCESSログ形式で、既定は無効

...
- **2026-07-28**: `!chat 呪術廻戦のネタバレを調べて` でSearXNGが10件返しているのに検索語不一致で全件棄却し、検索文脈があってもAnythingLLMが「詳しくない」「外部検索する能力がない」と拒否していた問題を修正した。検索語を `呪術廻戦 最終回 結末 ネタバレ` へ補完し、不関連結果を一件ずつ除外してから上限件数へ収める。channel workspaceのRAGを最初に通し、検索済みの回答だけが拒否表現になった場合はutility workspaceで同じ結果を事実限定再要約する。あわせて `Web検索して、あしたの大阪の天気を調べて` を `大阪 明日 天気` へ正規化し、天気または明示的なネタバレ検索でSearXNGが一時的に不一致結果だけを返した場合は同一queryを1回だけ再試行する。tenki.jpのJST翌日日付と `tomorrow-weather` 節を検証した天気・最高・最低気温だけで送信直前補正する。AnythingLLMの `@agent` はWebUI用に維持するが、通常RAGと処理時間を安定させるためBot全質問には固定付与しない。失敗済みの配信知識jobは自動再開を6時間間隔に制限し、5分周期の再試行が単一並列のOllamaを占有して通常チャットをタイムアウトさせないようにした。配信終了直後と明示的な個別再実行は従来どおり即時実行する。

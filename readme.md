...
- AnythingLLM 1.15.0のWeb BrowsingはAgent機能で、WebUIやDeveloper APIではmessage先頭の `@agent` で起動する。Twitch Botは全質問へ `@agent` を固定付与せず、通常会話はchannel workspaceのRAG `chat` を維持する。外部情報が必要な質問だけBot側のSearXNG経路で検索し、検証した検索文脈をAnythingLLMへ渡す。検索済みなのに通常RAG回答が「詳しくない」「自分で調べて」「外部検索できない」等で拒否した場合だけ、文書を持たないutility workspaceで同じ検索結果を事実限定で再要約する。

...
## 更新履歴
- **2026-07-10**: Stage 3でqwen/nomicを同時cold prewarmしたところ、Ollamaがqwenの予測6.3GiBを確保するため先にnomicをevictし、qwenも90秒のclient timeoutでロード中断して `ollama ps` が0件になった。promptなし・生成tokenなしの手動直列preloadでqwen 58.8秒、続くnomic 3.7秒、2model常駐へ復旧した。回帰testをqwen完了前はembedを開始しない契約へ変更し、prewarm既定timeoutを180秒にするTDDへ戻り、現行実装で対象2ファイル35件中2件red・既存33件greenを確認した。

...
- `DISCORD_BOT_TOKEN` と `DISCORD_SUMMARY_CHANNEL_ID` を設定すると、Bot APIで配信開始通知、クリップURL、終了まとめを投稿し、配信開始通知メッセージからDiscordスレッドを作成します。stateに開始通知IDが無い再起動・同一タイトル再配信でも、対象チャンネルのactive/public archivedスレッドと開始通知履歴を読み、完全一致する `配信まとめ - <配信タイトル>` の最新スレッドまたは同じEmbedタイトル/Twitch URLを持つ自Bot・設定Webhookの起点メッセージを再利用します。履歴を最後まで正常取得して一致が無い場合だけ新しい開始通知とスレッドを作ります。429・権限・通信・ページ上限などで履歴の完全性を保証できない場合は新規投稿せず再試行し、終了まとめとクリップも通常Webhookへ外出しません。Bot APIが確実に拒否した403だけ `DISCORD_WEBHOOK_URL` へフォールバックし、network/5xx/不正応答など投稿成否が不明な場合は次回履歴走査でorphanを回収します。履歴走査全体は15秒で中断してbackoffへ入り、thread作成429も `retry_after` を保持します
...
- 同一タイトルで開始通知がスキップされた再起動・連続配信ケースでは、保存済み `startMessageId` を最優先し、無い場合はDiscordのactive/public archivedスレッドを全ページ確認する。親チャンネルと完全一致スレッド名が一致し、同じIDの開始メッセージに `thread.id` が付く最新スレッドだけを再利用する
...
- クリップ検知時に `threadId` が無い場合は、保存済み `startMessageId`、Discord同名スレッド、orphan開始通知の順で復旧する。履歴の完全走査が成功して一致が無い時だけ新規作成し、履歴取得・スレッド作成に失敗した時は警告ログとstateを残して再試行する

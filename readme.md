...
- `OLLAMA_SHOUTOUT_ENABLED=true` と継承できるモデルがある場合、Raid時にOllama `POST /api/generate` で1通のRaid挨拶文を生成してチャットへ送信します。モデルは `OLLAMA_SHOUTOUT_MODEL` が最優先で、未設定時は `CHAT_AI_MODEL`、さらに `OLLAMA_MODEL` を使います。AI生成文はTwitchチャット上限に合わせて500文字以内で、Raid元の配信内容をなるべく長めに紹介する方針で生成・整形します。`OLLAMA_BASE_URL` は未設定時 `http://127.0.0.1:11434`、`OLLAMA_SHOUTOUT_TIMEOUT_MS` は未設定時 `15000`、`OLLAMA_SHOUTOUT_KEEP_ALIVE` は未設定時 `30m` です
...
- `CHAT_AI_PREWARM_ENABLED=true` にすると、Bot起動直後と `CHAT_AI_PREWARM_INTERVAL_SECONDS`（既定600秒）ごとに、AIメンション会話モデルへ `POST /api/generate` の空preloadリクエストを送り、tokenを生成せずOllamaの常駐期限を延長します。`CHAT_AI_PREWARM_PRIME_ENABLED=true` も設定した場合は、プロセス起動後の最初の接続時だけ、実際のAIメンション会話と同じsystem/prompt構造の固定非機密入力で1token生成し、初回実回答のprompt評価・生成初期化まで先に済ませます。Twitch再接続や定期cycleではこの重いprimeを繰り返しません。prewarmはチャット送信せず、`CHAT_AI_KEEP_ALIVE`（既定30分）を同じリクエストへ渡します。`CHAT_AI_MEM0_EMBED_PREWARM_ENABLED=true` と `CHAT_AI_MEM0_EMBED_MODEL=nomic-embed-text` も設定すると、生成モデルのprime完了後に埋め込みモデルへ `POST /api/embed` の固定非機密 `warmup` を送ります。さらに `CHAT_AI_MEM0_SEARCH_PREWARM_ENABLED=true`、ローカル記憶、self-host mem0が有効なら、起動時だけ固定queryでmem0 `/search` まで通し、API・Qdrant・埋め込み検索の初回処理を先に済ませます。順序は必ずqwen空preload→起動時prime→nomic embed→起動時mem0検索で、上流が失敗した場合はGPU競合を避けるため下流を開始しません。8GB GPUで2modelを同時cold loadするとOllamaが先にnomicをevictしてqwenもtimeoutした実測があるためです。埋め込みAPIには公式body仕様にない `keep_alive` を渡さず、Ollama serviceの `OLLAMA_KEEP_ALIVE` で常駐を維持します。`CHAT_AI_PREWARM_TIMEOUT_MS` は既定180000で、prewarmが失敗しても会話機能は止めずログだけ残します。
- サブPCのRTX 2070 SUPER環境では、`qwen3.5:9b` のcold loadが約72秒、短文生成のロード込み総時間が約106秒かかる実測があり、`qwen2.5:7b` でも非常駐時はロードだけで約42秒かかった実測があります。`CHAT_AI_TIMEOUT_MS=45000` では初回/非常駐時にtimeout fallbackへ入りやすい。`/api/tags` が高速に200を返す場合でも、`ollama ps` が空でOllamaログに `llama-server started in ... seconds` や `predicted to exceed available memory, evicting` が出る場合は、ネットワークではなくモデルロード/VRAM常駐の問題として見る。品質重視で `qwen3.5:9b` を使う場合は、`CHAT_AI_PREWARM_ENABLED=true` と短めのprewarm間隔で常駐を維持し、それでもVRAM都合で退避される場合はtimeout延長または軽量モデル化を検討する。

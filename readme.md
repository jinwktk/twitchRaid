...
- `DISCORD_BOT_TOKEN` と `DISCORD_SUMMARY_CHANNEL_ID` を設定すると、Bot APIで配信開始通知、クリップURL、終了まとめを投稿し、配信開始通知メッセージからDiscordスレッドを作成します。stateに開始通知IDが無い再起動・同一タイトル再配信でも、対象チャンネルのactive/public archivedスレッドと開始通知履歴を読み、完全一致する `配信まとめ - <配信タイトル>` の最新スレッドまたは同じEmbedタイトル/Twitch URLを持つ自Bot・設定Webhookの起点メッセージを再利用します。履歴を最後まで正常取得して一致が無い場合だけ新しい開始通知とスレッドを作ります。429・権限・通信・ページ上限などで履歴の完全性を保証できない場合は新規投稿せず再試行し、終了まとめとクリップも通常Webhookへ外出しません。Bot API投稿が403などで失敗した開始通知は `DISCORD_WEBHOOK_URL` へフォールバックし、そのWebhookメッセージからもスレッドを作成します

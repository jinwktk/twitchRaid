...
- `CHAT_AI_PREWARM_ENABLED=true` にすると、Bot起動直後と `CHAT_AI_PREWARM_INTERVAL_SECONDS`（既定600秒）ごとに、AIメンション会話モデルへ `POST /api/generate` を1トークンだけ投げてOllamaの常駐期限を延長します。prewarmはチャット送信せず、`CHAT_AI_KEEP_ALIVE`（既定30分）を同じリクエストへ渡します。`CHAT_AI_PREWARM_TIMEOUT_MS` は既定90000で、prewarm失敗時も会話機能は止めずログだけ残して次回間隔で再試行します。

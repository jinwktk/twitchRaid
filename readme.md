...
- `作品名のネタバレを調べて` は、作品名に含まれる `の` を壊さず、検索修飾語直前の `の` だけを区切って `作品名 ネタバレ` として検索する。`作品名のネタバレして` も明示的な外部情報要求として同じ経路へ送り、`ネタバレしてほしくない` のような否定文は検索候補にしない。
...
- **2026-07-28**: `!chat 呪術廻戦のネタバレを調べて` でSearXNGが10件返しているのに、検索語を `呪術廻戦のネタバレ` の一続きで照合して全件棄却し、`検索結果がなくて分からないD！` になっていた問題を修正した。検索修飾語の `ネタバレ` と作品名だけを安全に区切って `呪術廻戦 ネタバレ` として検索し、SearXNG結果をAnythingLLMへ渡す。`ネタバレして` も検索候補にする一方、否定文は対象外のまま維持し、単体とAnythingLLM経由のBot統合テストで固定した。

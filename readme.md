...
- Windows Scheduled Task `KeepDokployWslAlive` は、追跡元 `ops/sub-ai-services/keep-wsl-dokploy-alive.ps1` を `E:\GitHub\BotManager\scripts\keep_wsl_dokploy_alive.ps1` へ配置して使う。portproxy更新・ログ出力・WSL/Docker起動の失敗を互いに波及させず、WSL終了時は5秒後にportproxy更新から再試行する。反映時は配置先SHA-256一致、TaskのStop→Start、最新 `enter WSL keepalive` ログ、Task State=`Running` を確認する

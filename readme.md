...
- Windows PowerShellの `trmem-web` または `npm run memory:web` で起動する `twitchRaid Memory` WebUIは、既存のURL/portを維持したままmem0を管理します。メインPC起動版は `http://127.0.0.1:3220/` で待ち受け、操作時に `ssh sub` 経由でSUB AI Services内の `sub-ai_mem0` コンテナへ入り、`MEM0_ADMIN_API_KEY` を外へ出さずに内部REST APIを呼びます。サブPC常駐版は `http://192.168.0.99:3220/` で、WebUI専用Swarm service `twitchraid-memory-web` が `dokploy-network` 上の `http://mem0:8888` へdirect接続します。画面ではscope `CHAT_AI_MEM0_USER_ID` / `CHAT_AI_MEM0_AGENT_ID` の一覧、semantic search、新規追加、EditによるKey/本文更新、Deleteによるmem0 ID削除ができます。Editは既存mem0 IDを維持し、本文だけでなく `metadata.key` / `metadata.kind` も更新します。mem0にはinactive状態がないため、旧SQLite画面のactive/inactive切替は廃止しています。`npm run memory:web:direct` は `--endpoint` と `--api-key` で直接mem0 wrapperへ接続する検証用です。PowerShell登録先は `C:\Users\mlove\Documents\PowerShell\Microsoft.PowerShell_profile.ps1` です。
...
- `ops/sub-ai-services/docker-compose.yml` のAnythingLLMは`mintplexlabs/anythingllm:1.15.0`とdigestを固定し、外部portを公開しない。serviceの所有元は1つに限定し、同じstorageを手動serviceとCompose serviceから同時mountしない
...
- 既存SQLite/mem0記憶は、shadow-write確認後に運用者が`npm run migrate:anythingllm-memory`で一度だけ移行する。移行snapshotは専用SQLiteへ先に固定されるため再実行可能で、activeかつ安全な記憶だけを無期限文書へ入れる。候補・無効・削除済みkeyは復活させず、本文や秘密値をConsoleへ出さない
...
- `CHAT_AI_ANYTHINGLLM_ENABLED=true` では、Botが構築した安全ラベル付きpromptをAnythingLLMのstable channel sessionへ送り、AnythingLLMがOllamaを呼ぶ。無効時だけ従来互換としてBotからOllama `POST /api/generate` を直接使う。どちらの経路でも、短く済む時は短く、説明が必要な時は最大 `CHAT_AI_MAX_RESPONSE_CHARS` 文字内で複数文を許可し、最終的には単一行へ整形する。英語一般語が残る候補や歌唱契約違反は専用utility sessionで1回だけ修正し、なお不正なら送信しない。`GG！` のような短い表現、固有名詞、Twitch表示名は許可し、login IDでの呼びかけは表示名へ置換する。勝敗質問の低情報返答は画面を見ていない安全な定型文へフォールバックし、`CHAT_REPLY_EMOTES` は500文字内で末尾へ付ける
...
- 外部検索は `CHAT_AI_SEARCH_ENABLED=true` の場合だけ使う。検索・調べて・最新・ニュース・誰/いつ/どこ、`夏尾さんについて` や `夏尾さんについて知ってる？` のような `〜について`、日程/開催/主催/開発/リリース日など外部事実が必要な質問に限定し、明示的な記憶依頼は検索しない。`教えて` / `わからない` は単独では検索候補にせず、外部事実語と組み合わさる時だけ検索候補にする。既定providerは `duckduckgo` で、SearXNGを使う場合は `CHAT_AI_SEARCH_PROVIDER=searxng`、`CHAT_AI_SEARCH_ENDPOINT=http://searxng:8080/search?language=all&safesearch=0`、必要に応じて `CHAT_AI_SEARCH_ENGINES=google` を設定する。`ops/sub-ai-services/docker-compose.yml` はSearXNGをSUB AI Services stack内の1サービスとして置くための雛形で、SearXNG自体は公開ポートなし、`dokploy-network` の内部DNS alias `searxng` でBotから接続する。SearXNG設定ファイルは `ops/searxng/settings.yml` を雛形にし、サブPC本番では `/home/mlove/dokploy/searxng/settings.yml` に置く。外向きHTTP接続は `keepalive_expiry=300` で再利用し、検索結果やengine構成はキャッシュしない。検索語は文末の `について調べて` 等を落としてから送り、`るっかるん` を含む検索語には `rukalun` を補う。URL、メール、電話番号、token/API key/password系を含む検索語や長すぎる検索語は外部へ送らない。検索結果は「命令ではない参考情報」としてプロンプトへ入れ、検索結果がある場合は事実情報として優先して要約するようOllamaへ指示する。HTTP失敗、壊れたJSON、空結果、過大レスポンス時は検索なしで通常返信へ戻す。検索候補ではない通常質問でも、検索文脈なしのOllama生成が不明系返信になった時だけ、同じ安全条件で1回だけ強制検索し、結果があれば再生成する。検索候補なのに検索が使われなかった場合は、`AIメンション会話外部検索は未適用: reason=disabled` または `reason=no_result_or_failed` をINFOログへ残し、二段階リサーチで検索できた場合は `AIメンション会話リサーチ検索を適用: results=N` を残す
...
- mem0連携は `CHAT_AI_MEM0_ENABLED=true` と `CHAT_AI_MEM0_ENDPOINT` がある場合だけ使う任意の補助記憶である。費用を絶対に発生させないため、接続先はSUB AI Services内のself-host Mem0 OSS REST APIだけにし、Mem0 Platformのhosted endpointは拒否する。`ops/mem0-oss-server/` は `mem0ai` Python OSSライブラリをFastAPIで薄く包む内部REST wrapperで、`/memories` / `/search` / `/healthz` と管理用 `PATCH /memories/{id}` / `DELETE /memories/{id}` を提供する。wrapperは起動時に共有 `Memory` instanceを初期化し、依存先が未準備なら起動自体は継続して最初のAPI要求で再試行する。Qdrantは公開ポートなし、`QDRANT__TELEMETRY_DISABLED=true`、mem0はOllama `nomic-embed-text:latest` を使う。通常会話ではmem0 semantic searchの結果をローカルメモと結合するが、ローカルJSON/SQLite正本に同一 `key: value` または同一キーがあるmem0行は除外して正本を優先する。明示メモとactive化した暗黙メモだけ、同じ抽出済み内容を `infer:false` でmem0へ保存し、candidateは送らない。scopeは `CHAT_AI_MEM0_USER_ID` / `CHAT_AI_MEM0_AGENT_ID` / `CHAT_AI_MEM0_APP_ID` で固定する。mem0障害時は会話とローカル保存を止めず、ログは `AIメンション会話mem0メモを適用: items=...`、重複除外時は `AIメンション会話mem0メモ重複を除外: items=...`、未適用時は `reason=...` に限定する。`scripts/memory-web.mjs` は既存の `twitchRaid Memory` WebUIの内部データソースをmem0へ切り替え、`ssh sub` 経由で `sub-ai_mem0` の内部REST APIを呼ぶ
...
└── memory-web.mjs                 # 既存Memory WebUIのmem0管理バックエンド
ops/
